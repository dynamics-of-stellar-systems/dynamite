...
- Improvement: the NNLS weight solve with ``nnls_solver = adelie`` no longer materializes the classic constraint matrix A. The augmented, column-scaled matrix X is assembled directly from the orbit library, halving resident memory during the solve (measured on an omega Cen library: peak RSS 626 -> 502 GiB at float64). The final chi2 and KKT certificate are computed from the solver's residual via algebraically identical identities. Setting ``stream_orblib_reads: true`` additionally reads orbit-library histograms one kinematic set at a time (peak 380 GiB at float64, 190 GiB with ``nnls_dtype: float32``), enabling several concurrent weight solves per node; streamed construction is bit-identical to non-streamed.
- Fix: streaming histogram reads initially desynchronized the bulk parsers' record walk when one file stores the apertures of several kinematic sets (the parsers require ``ap_global`` to enumerate every aperture in the file). The parsers now take a ``keep`` mask over the full per-file aperture list instead; see ``dev_tests/test_streamed_parser_subset.py``.
- Improvement: orbit libraries with ``ProperMotions`` kinematics can now be integrated in chunks. The proper-motion ``pm_hist`` stream is merged alongside ``qgrid`` and ``losvd_hist``; it carries no header record, since ``output_setup`` creates that file without a setup writer. Which streams a library writes is now decided per library, so a proper-motion-only library merges correctly too. Only populations data still forces single-process integration.
- Bugfix: ``orblib_chunks`` > 1 produced an orbit library short by a factor ``dithering``:math:`^3`, because the number of orbit starting points was divided by ``dithering``:math:`^3` a second time - the legacy Fortran already applies that factor before counting starting points. Reading such a library failed with ``Size obtained (1) is not a multiple of the dtypes given (4)``. The bug was invisible at ``dithering: 1``. The merge now also verifies that the chunks together hold every orbit, rather than trusting the library header, which states the full size whatever range a process was given.
- Bugfix: a chunked orbit library did not produce the merged ``orblib.dat_orbclass.out`` and ``orblibbox.dat_orbclass.out`` files, which are needed for orbit-property and circularity (:math:`\lambda_z`) analysis and plotting, and did not mark itself complete, so it was integrated again by every model that needed it.
- Bugfix: orbit library archives are now compressed to a temporary name and renamed. ``orblibs_in_parallel: True`` rewrote the ``.bz2`` files in place, so a model reading a library while another model rebuilt it could decompress a truncated archive.
- Bugfix: chunking is now skipped for ``LegacyWeightSolver`` and for libraries restricted via ``starting_orbit``/``number_orbits``, neither of which the chunked path can build correctly. ``orblib_chunks: auto`` no longer oversubscribes the ``total_cores`` budget by a factor 2 when ``orblibs_in_parallel`` is ``False``.
- Bugfix: a failed orbit library read no longer leaves the process in the model directory, which caused unrelated models in the same worker to fail afterwards, and no longer leaves the decompressed library on disk. Damaged, truncated and stale library files are now reported with the file name and what to delete, instead of surfacing as errors from inside ``scipy.io.FortranFile`` or as a bare ``IndexError``.
- Bugfix: initial conditions are regenerated when :math:`(n_E, n_{I2}, n_{I3})` or ``dithering`` change, instead of silently reusing ``begin.dat`` from the previous orbit grid.
- Bugfix: a model waiting for another model to finish building a shared orbit library no longer rebuilds it when the build completes inside one poll interval, and no longer gives up after an hour while the build is still making progress.
