...
!NOT95USE GALAHAD_CPU_time
      ...
      USE HSL_ZD11_double
      USE HSL_ZD11_double, ONLY: MA48_type => ZD11_type
...
!-------------------------------------------------
      ...
      TYPE, PUBLIC :: SBLS_control_type
        ...
        INTEGER :: indmin, valmin, len_ulsmin, itref_max, scaling
      ...
      TYPE, PUBLIC :: SBLS_explicit_factors_type
        ...
        TYPE ( ZD11_type ) :: K
        TYPE ( MA48_type ) :: B
        TYPE ( MA57_factors ) :: K_factors
        TYPE ( MA48_factors ) :: B_factors
        TYPE ( MA57_control ) :: K_control
        TYPE ( MA48_control ) :: B_control
      ...
      TYPE, PUBLIC :: SBLS_implicit_factors_type
        ...
        TYPE ( ZD11_type ) :: A2, P11, P21, B22, B32, B33
        TYPE ( MA48_type ) :: A1, B31, P22
        TYPE ( MA48_factors ) :: A1_factors, B31_factors, P22_factors
        TYPE ( MA48_control ) :: A1_control, B31_control, P22_control
        TYPE ( MA57_factors ) :: B22_factors
        TYPE ( MA57_control ) :: B22_control
      ...
      TYPE, PUBLIC :: SBLS_null_space_factors_type
        PRIVATE
        INTEGER :: rank_a, m, n, k_n, len_sol_workspace, n_r
        TYPE ( ZD11_type ) :: A2, H11, H21, H22, R_sparse
        TYPE ( MA48_type ) :: A1
        TYPE ( MA48_factors ) :: A1_factors
        TYPE ( MA48_control ) :: A1_control
        TYPE ( MA57_factors ) :: R_sparse_factors
        TYPE ( MA57_control ) :: R_sparse_control
        INTEGER, ALLOCATABLE, DIMENSION( : ) :: A_ROWS_order
        INTEGER, ALLOCATABLE, DIMENSION( : ) :: A_COLS_order
        INTEGER, ALLOCATABLE, DIMENSION( : ) :: A_ROWS_basic
        INTEGER, ALLOCATABLE, DIMENSION( : ) :: A_COLS_basic
      ...
      TYPE, PUBLIC :: SBLS_inform_type
        ...
        INTEGER :: uls_analyse_status, uls_solve_status, sort_status
...
!   
!   indmin. An initial guess as to the integer workspace required by MA57
!
!   valmin. An initial guess as to the real workspace required by MA57
! 
!   len_ulsmin. An initial guess as to the workspace required by MA48
!
...
!    constraints
!
!   static_tolerance & static_level (may be) used by MA57
!
...
! =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ...
      TYPE ( SBLS_data_type ), INTENT( OUT ) :: data
...
!  Initialize control parameters and arrays used within MA57 and M48

      CALL MA48_INITIALIZE( data%efactors%B_factors,                            &
                           data%efactors%B_control )
      CALL MA48_INITIALIZE( data%ifactors%A1_factors,                           &
                           data%ifactors%A1_control )
      CALL MA48_INITIALIZE( data%nfactors%A1_factors,                           &
                           data%nfactors%A1_control )
      CALL MA57_INITIALIZE( data%efactors%K_factors,                           &
                            data%efactors%K_control )
      CALL MA57_INITIALIZE( data%ifactors%B22_factors,                         &
                            data%ifactors%B22_control )
      CALL MA57_INITIALIZE( data%nfactors%R_sparse_factors,                    &
...
!  Set integer values
      ...
      CALL SPECFILE_assign_value( spec( 3 ), control%print_level,              &
                                  control%error )
      CALL SPECFILE_assign_value( spec( 8 ), control%len_ulsmin,               &
                                  control%error )
...
!  Deallocate all arrays allocated within MA57 and MA48

      CALL MA57_finalize( data%efactors%K_factors,                             &
      ...
      CALL MA48_finalize( data%efactors%B_factors,                              &
                         data%efactors%B_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

      CALL MA48_finalize( data%ifactors%A1_factors,                             &
                         data%ifactors%A1_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

!     CALL MA48_finalize( data%ifactors%B31_factors,                            &
!                        data%ifactors%B31_control, inform%alloc_status )
!     IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
!       inform%status = GALAHAD_error_deallocate ; RETURN
!     END IF

!     CALL MA48_finalize( data%ifactors%P22_factors,                            &
!                         data%ifactors%P22_control, inform%alloc_status )
!     IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
!       inform%status = GALAHAD_error_deallocate ; RETURN
!     END IF

      CALL MA57_finalize( data%ifactors%B22_factors,                           &
                          data%ifactors%B22_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

      CALL MA48_finalize( data%nfactors%A1_factors,                             &
                         data%nfactors%A1_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

      CALL MA57_finalize( data%nfactors%R_sparse_factors,                      &
...
!  -4 MA57 analyse error
!  -5 MA57 factorize error
!  -6 MA57 solve error
!  -7 MA48 analyse error
!  -8 MA48 solve error
...
!  Dummy arguments
      ...
      TYPE ( ZD11_type ), INTENT( IN ) :: H, A, C
      TYPE ( SBLS_data_type ), INTENT( INOUT ) :: data
      TYPE ( SBLS_control_type ), INTENT( IN ) :: control
      TYPE ( SBLS_inform_type ), INTENT( INOUT ) :: inform
...
!  Set default information values
      ...
      inform%uls_analyse_status = 0 ; inform%uls_solve_status = 0
...
!  Ensure automatic choices for the preconditioner/factorization have been made
      ...
      IF ( data%last_preconditioner /= control%preconditioner ) THEN
        IF ( control%preconditioner == 0 ) THEN
         IF ( data%last_preconditioner == no_last ) inform%preconditioner = 1
        ELSE
      ...
      IF ( data%last_factorization /= control%factorization ) THEN
        IF ( control%factorization == 0 ) THEN
          IF ( data%last_factorization == no_last ) inform%factorization = 1
        ELSE
...
!  Only allow the null-space method if C = 0
      ...
      IF ( inform%factorization == 3 ) THEN
        IF ( ZD11_get( C%type ) == 'ZERO' ) THEN
          c_ne = 0
        ELSE IF ( ZD11_get( C%type ) == 'DIAGONAL' ) THEN
          c_ne = m
        ELSE IF ( ZD11_get( C%type ) == 'DENSE' ) THEN
          c_ne = ( m * ( m + 1 ) ) / 2
        ELSE IF ( ZD11_get( C%type ) == 'SPARSE_BY_ROWS' ) THEN
          c_ne = C%ptr( m + 1 ) - 1
        ELSE
          c_ne = C%ne 
        END IF
...
!  Local variables
      ...
      CHARACTER ( LEN = 80 ) :: array_name
      TYPE ( MA57_AINFO ) :: AINFO_MA57
      TYPE ( MA57_FINFO ) :: FINFO_MA57
      TYPE ( MA48_AINFO ) :: AINFO_MA48
      TYPE ( MA48_FINFO ) :: FINFO_MA48
...
!  prefix for all output 
      ...
      IF ( ZD11_get( A%type ) == 'DENSE' ) THEN
        a_ne = m * n
      ELSE IF ( ZD11_get( A%type ) == 'SPARSE_BY_ROWS' ) THEN
        a_ne = A%ptr( m + 1 ) - 1
      ELSE
        a_ne = A%ne 
      END IF

      IF ( ZD11_get( H%type ) == 'DIAGONAL' ) THEN
        h_ne = n
      ELSE IF ( ZD11_get( H%type ) == 'DENSE' ) THEN
        h_ne = ( n * ( n + 1 ) ) / 2
      ELSE IF ( ZD11_get( H%type ) == 'SPARSE_BY_ROWS' ) THEN
        h_ne = H%ptr( n + 1 ) - 1
      ELSE
        h_ne = H%ne 
      END IF
...
!  for storage by rows!)
          ...
          IF ( ZD11_get( A%type ) == 'COORDINATE' ) THEN
            array_name = 'sbls: efactors%A_row_ptr'
...
!  Find sets of basic rows and columns
            ...
            CALL SBLS_find_basis( m, n, a_ne, A, efactors%B,                   &
                                  ...
                                  control, inform, AINFO_MA48, FINFO_MA48 )
 ...
 200  CONTINUE
      ...
      IF ( printi ) THEN
        IF ( inform%factorization == 1 ) THEN
          WRITE( out,                                                          &
            "( /, A, ' Using MA57 to factorize the Schur complement' )" ) prefix
        ELSE
          WRITE( out,                                                          &
            "( /, A, ' Using MA57 to factorize the augmented matrix' )" ) prefix
        END IF
      END IF
...
!       WRITE( out, "( ' n, nnz ', I7, I10 )" ) efactors%K%n, efactors%K%ne
        ...
        END IF
        efactors%K_control%ordering = control%ordering
        efactors%K_control%scaling = control%scaling
        efactors%K_control%static_tolerance = control%static_tolerance
        efactors%K_control%static_level = control%static_level
...
!  Analyse the preconditioner
        ...
        IF ( efactors%K%n > 0 ) THEN
          CALL MA57_ANALYSE( efactors%K, efactors%K_factors,                   &
                             efactors%K_control, AINFO_MA57 )
          inform%sils_analyse_status = AINFO_MA57%flag
          IF ( printi ) WRITE( out,                                            &
             "(  A, ' MA57: analysis complete:      status = ', I0 )" )        &
...
!  Factorize the preconditioner
      ...
      IF ( efactors%K%n > 0 ) THEN
        ...
        CALL MA57_FACTORIZE( efactors%K, efactors%K_factors,                   &
                             efactors%K_control, FINFO_MA57 )
        inform%sils_factorize_status = FINFO_MA57%FLAG
        IF ( printi ) WRITE( out,                                              &
          "( A, ' MA57: factorization complete: status = ', I0 )" ) prefix,    &
        ...
        IF ( inform%sils_factorize_status == 4 ) THEN
          ...
          inform%rank = FINFO_MA57%rank
        END IF

        IF ( printi ) WRITE( out, "( A, ' K nnz(prec,factors)', 2( 1X, I0 ) )")&
          prefix, efactors%K%ne, FINFO_MA57%NEBDU

        kzero = efactors%K%n - FINFO_MA57%rank
        kminus = FINFO_MA57%NEIG
...
!  Now factorize H_22
        ...
        IF ( printi ) WRITE( out, "( A, ' Using MA57' )" ) prefix
        IF ( control%print_level <= 0 ) THEN
        ...
        END IF
        ifactors%B22_control%ordering = control%ordering
        ifactors%B22_control%scaling = control%scaling
        ifactors%B22_control%static_tolerance = control%static_tolerance
        ifactors%B22_control%static_level = control%static_level
        IF ( control%perturb_to_make_definite ) ifactors%B22_control%pivoting = 4
        CALL MA57_ANALYSE( ifactors%B22, ifactors%B22_factors,                 &
                           ifactors%B22_control, AINFO_MA57 )
        inform%sils_analyse_status = AINFO_MA57%FLAG
        IF ( printi ) WRITE( out,                                              &
       "( A, ' Analysis complete:      status = ', I0 )" ) prefix,             &
        ...
        CALL MA57_FACTORIZE( ifactors%B22, ifactors%B22_factors,               &
                             ifactors%B22_control, FINFO_MA57 )
        inform%sils_factorize_status = FINFO_MA57%FLAG
        IF ( printi ) WRITE( out,                                              &
...
!  Check to ensure that the preconditioner is definite
        ...
        IF ( FINFO_MA57%modstep /= 0 ) THEN
          ...
          CALL MA57_ENQUIRE( ifactors%B22_factors, PERTURBATION = ifactors%PERT )
        ...
        IF ( FINFO_MA57%NEIG + ifactors%B22%n - FINFO_MA57%rank > 0 ) THEN
          WRITE( out, "( A, ' MA57_FACTORIZE reports B22 is indefinite ' )" )  &
            prefix  
           inform%status = GALAHAD_error_preconditioner ; RETURN
        END IF
        IF ( printi ) WRITE( out, "( A, ' B22 nnz(prec,factors)', 2( 1X, I0))")&
          prefix, ifactors%B22%ne, FINFO_MA57%nrlbdu
...
!       write(6,*) ' resize '
        array_name = 'sbls: efactors%RHS'
        CALL SPACE_resize_array( n + m, efactors%RHS,                          &
           inform%status, inform%alloc_status, array_name = array_name,        &
           deallocate_error_fatal = control%deallocate_error_fatal,            &
           exact_size = .TRUE.,                                                &
           bad_alloc = inform%bad_alloc, out = control%error )
...
!           WRITE(6,*) efactors%RHS( np1 : npm )
            CALL MA57_SOLVE( efactors%K, efactors%K_factors,                   &
              efactors%RHS( np1 : npm ), efactors%K_control, SINFO_MA57 )
            inform%sils_solve_status = SINFO_MA57%flag
...
!  Use factors of the augmented system
          ...
          CALL MA57_SOLVE( efactors%K, efactors%K_factors, efactors%RHS,       &
                           efactors%K_control, SINFO_MA57 )
          inform%sils_solve_status = SINFO_MA57%flag
          IF ( inform%sils_solve_status < 0 ) THEN
            IF ( control%out > 0 .AND. control%print_level > 0 )               &
              WRITE( control%out, "( A, ' solve exit status = ', I3 )" )       &
...
!  1c. Solve A^1^T v_3 = r_1
        ...
        trans = 1
        CALL MA48_SOLVE( ifactors%A1, ifactors%A1_factors,                      &
                        SOL( start_1 : end_1 ),                                &
                        ifactors%SOL_perm( start_3 : end_3 ),                  &
                        ifactors%A1_control, SINFO_MA48, TRANS = trans )
        inform%uls_solve_status = SINFO_MA48%flag
...
!  2c. Solve B_22 v_2 = r_2
        ...
        IF ( .NOT. ifactors%unitb22 ) THEN
          CALL MA57_SOLVE( ifactors%B22, ifactors%B22_factors,                &
            SOL( start_2 : end_2 ),  ifactors%B22_control, SINFO_MA57 )
          inform%sils_solve_status = SINFO_MA57%flag
          IF ( inform%sils_solve_status < 0 ) THEN
            IF ( control%out > 0 .AND. control%print_level > 0 )               &
              WRITE( control%out, "( A, ' solve exit status = ', I3 )" )       &
...
!  3c. Solve A_1 v_1 = r_3
        ...
        CALL MA48_SOLVE( ifactors%A1, ifactors%A1_factors,                      &
                        SOL( start_3 : end_3 ),                                &
                        ifactors%SOL_perm( start_1 : end_1 ),                  &
                        ifactors%A1_control, SINFO_MA48 )
        inform%uls_solve_status = SINFO_MA48%flag
        IF ( inform%uls_solve_status < 0 ) THEN
          inform%status = GALAHAD_error_uls_solve ; RETURN
        END IF
...
!  Reorder A2 so that the matrix is stored by columns (its transpose by rows)
        ...
        IF ( nfactors%A2%ne > 0 ) THEN
         ...
         "( A, ' MA48: sort of A2, info = ', I0 )" )  prefix, inform%sort_status
...
!  Finally. reorder H21 and H22 so that they are stored by rows
        ...
        IF ( nfactors%H21%ne > 0 ) THEN
          ...
          IF ( inform%sort_status /= 0 ) THEN
            IF ( printe ) WRITE( error,                                        &
         "( A, ' MA48: sort of H21, info = ', I0 )" )  prefix, inform%sort_status
            IF ( inform%sort_status > 0 ) THEN
              inform%status = GALAHAD_error_sort ; RETURN
            END IF
          END IF
        ...
        IF ( nfactors%H22%ne > 0 ) THEN
          ...
          IF ( inform%sort_status /= 0 ) THEN
            IF ( printe ) WRITE( error,                                        &
         "( A, ' MA48: sort of H22, info = ', I0 )" )  prefix, inform%sort_status
            IF ( inform%sort_status > 0 ) THEN
              inform%status = GALAHAD_error_sort ; RETURN
            END IF
          END IF
...
!  2. Now form A_1^-1 v -> w
          ...
          CALL MA48_SOLVE( nfactors%A1, nfactors%A1_factors,                   &
                          nfactors%V( : nfactors%rank_a ),                    &
                          nfactors%W( : nfactors%rank_a ),                    & 
                          nfactors%A1_control, SINFO_MA48 )
          inform%uls_solve_status = SINFO_MA48%flag
          IF ( inform%uls_solve_status < 0 ) THEN
...
!  5. Next form w = A_1^-T v
          ...
          CALL MA48_SOLVE( nfactors%A1, nfactors%A1_factors,                    &
                          nfactors%V( : nfactors%rank_a ),                     &
                          nfactors%W( : nfactors%rank_a ),                     &
                          nfactors%A1_control, SINFO_MA48, TRANS = trans )
          inform%uls_solve_status = SINFO_MA48%flag
          IF ( inform%uls_solve_status < 0 ) THEN
...
!  Dummy arguments
      ...
      TYPE ( MA48_type ), INTENT( inout ) :: A1
      TYPE ( ZD11_type ), INTENT( INOUT ) :: A2
      TYPE ( MA48_factors ), INTENT( INOUT ) :: A1_factors
      TYPE ( MA48_control ), INTENT( INOUT ) :: A1_control
      ...
      TYPE ( SBLS_control_type ), INTENT( IN ) :: control
      TYPE ( SBLS_inform_type ), INTENT( INOUT ) :: inform
      TYPE ( MA48_AINFO ), INTENT( INOUT ) :: AINFO_MA48
      TYPE ( MA48_FINFO ), INTENT( INOUT ) :: FINFO_MA48
...
!  Factorize A1
      ...
      CALL MA48_finalize( A1_factors, A1_control, info )
      IF ( control%deallocate_error_fatal .AND.                                &
           inform%status /= GALAHAD_ok ) RETURN
      CALL MA48_INITIALIZE( A1_factors, A1_control )
      IF ( control%print_level <= 0 ) THEN
        A1_control%ldiag = 0
      ...
      END IF
      A1_control%la = control%len_ulsmin
      A1_control%fill_in = 3

      CALL MA48_ANALYSE( A1, A1_factors, A1_control, AINFO_MA48, FINFO_MA48 )
      inform%uls_analyse_status = AINFO_MA48%flag
      IF ( printi ) WRITE( out,                                                &
         "( A, ' MA48: analysis of A1 complete: status = ', I0 )" )             &
             prefix, inform%uls_analyse_status
      IF ( inform%uls_analyse_status < 0 ) THEN
         inform%status = GALAHAD_error_uls_analysis ; RETURN
      END IF
      IF ( inform%uls_analyse_status == 4 ) THEN
        inform%rank_def = .TRUE.
        inform%rank = AINFO_MA48%rank
      END IF

      IF ( printi ) WRITE( out, "( A, ' A1 nnz(prec,factors)', 2( 1X, I0 ))")  &
        prefix, A1%ne, FINFO_MA48%size_factor
...
!  approximation to e
      ...
      IF ( control%check_basis ) THEN
        ...
        CALL MA48_SOLVE( A1, A1_factors, RHS( : rank_a ), SOL( : rank_a ),      &
                        A1_control, SINFO_MA48 )
        inform%uls_solve_status = SINFO_MA48%flag
!       write(6,*) ' uls solve ', inform%uls_solve_status
!       write(6,"( A, /, ( 5ES12.4 ) )" ) ' uls solve - solution ', SOL( : 5 )
        IF ( inform%uls_solve_status < 0 ) THEN
...
!       CALL CPU_TIME( t3 )
        CALL MA48_ANALYSE( A1, A1_factors,                  &
                          A1_control, AINFO_MA48, FINFO_MA48 )
!       CALL CPU_TIME( t2 )

        inform%uls_analyse_status = AINFO_MA48%flag
        IF ( printi ) WRITE( out,                                              &
           "( /, A, ' MA48: analysis of A  complete: status = ', I0 )" )        &
               prefix, inform%uls_analyse_status
...
!       CALL CPU_TIME( t3 )
        CALL MA48_SPECIAL_ROWS_AND_COLS( A1_factors, rank_a, A_COLS_basic,      &
                                        A_ROWS_basic, info )
!       CALL CPU_TIME( t2 )
...
!  Determine the nonsingular submatrix of the factorization
        ...
        CALL MA48_SPECIAL_ROWS_AND_COLS( A1_factors, rank_a, A_ROWS_basic,      &
                                        A_COLS_basic, info )
      END IF    
