...
C
 200  CONTINUE
      ...
      IF ( VARN .EQ. 'V' ) THEN
        CLS(22:22) = VARN
      ELSE
        WRITE ( CLS(18:22), '( I5 )' ) NVAR
      ENDIF
      CLS(23:23) = '-'
      IF ( VARM .EQ. 'V' ) THEN
        CLS(28:28) = VARM
      ELSE
        WRITE ( CLS(24:28), '( I5 )' ) NCON
      ENDIF
C
...
C
 900  CONTINUE
      CLOSE ( DBDVC )
      IF ( .NOT. WRITTN ) WRITE ( UDBDVC, '( A28 )' ) CLS
      CLOSE ( UDBDVC )
      STOP
C
