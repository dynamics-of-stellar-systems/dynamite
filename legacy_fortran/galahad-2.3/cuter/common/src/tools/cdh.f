C     ( Last modified on 23 Dec 2000 at 22:01:38 )
      ...
      INTEGER            N , M , LV    , LH1
CS    REAL               X( N ), V( LV ), H( LH1, N )
CD    DOUBLE PRECISION   X( N ), V( LV ), H( LH1, N )
C
...
C    X and V. The i,j-th component of the array will contain
...
C
     ...
     *             1, IFSTAT )
C
C  evaluate the element function Hessian values.
C
      CALL ELFUN ( FUVALS, X, WK( EPVALU + 1 ), NELNUM,
     *             IWK( ITYPEE + 1 ), IWK( ISTAEV + 1 ),
     *             IWK( IELVAR + 1 ), IWK( INTVAR + 1 ),
