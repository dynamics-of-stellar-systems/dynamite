...
CD    DOUBLE PRECISION   X     ( N      ), V     ( LV     ), 
     *                   HI    ( LHI    ), CJAC  ( LCJAC  )
C
C  Compute the Jacobian matrix for an optimization problem
C  initially written in Standard Input Format (SIF).
C  Also compute the Hessian matrix of the Lagrangian function of
C  the problem.
C
C  The Jacobian is represented in "co-ordinate" format.
C  The Hessian is represented in "finite element format", i.e., 
C
C           ne
C      H = sum H_i, 
C          i=1
...
CS    EXTERNAL           SSETVL, SSETVI, SELGRD, SASMBE
CD    EXTERNAL           DSETVL, DSETVI, DELGRD, DASMBE
C
...
C
      IF ( NNZJ .GT. LCJAC ) THEN
         IF ( IOUT .GT. 0 ) WRITE( IOUT, 2030 ) NNZJ - LCJAC 
         STOP
      END IF
C
