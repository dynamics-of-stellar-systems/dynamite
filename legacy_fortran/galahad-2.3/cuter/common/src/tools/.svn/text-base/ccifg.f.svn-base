...
C  The constraint gradient is stored as a dense vector in array GCI;
C  that is, GCI( j ) is the partial derivative of constraint ICON
C  with respect to variable j.
C  (Subroutine CSCIFG performs the same calculations for a sparse
...
C
   ...
   10 CONTINUE
C
C  Evaluate the element functions.
C
...
C
C  Update the constraint function evaluation counter
C
      NC2CF = NC2CF + 1
      END IF
      IF ( GRAD ) THEN
C
C  Update the constraint gradient evaluation counter
C
      NC2CG = NC2CG + 1
C
...
C
     ...
     *                2, IFSTAT )
C
C  Evaluate the group derivative.
C
     ...
     *                   .TRUE., IGSTAT )
C
C  Compute the gradient.  Initialize the gradient vector as zero.
C
         DO 120 J = 1, N
            GCI( J ) = ZERO
  120    CONTINUE
...
C
               IF ( LL .LE. N ) GCI( LL ) = GI * WK( WRK + LL )
  190       CONTINUE
C
