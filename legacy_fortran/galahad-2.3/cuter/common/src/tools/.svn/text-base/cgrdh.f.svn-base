...
CS    REAL               X     ( N    ), G     ( N    ), V     ( LV ),
CD    DOUBLE PRECISION   X     ( N    ), G     ( N    ), V     ( LV ),
     *                   CJAC  ( LCJAC1, LCJAC2       ),
     *                   H     ( LH1   , N            )
C
C  Compute both the gradients of the objective, or Lagrangian, and
C  general constraint functions and the Hessian matrix of the
C  Lagrangian function of a problem initially written in Standard
C  Input Format (SIF).
C
...
C        Hessian matrix of the Lagrangian function evaluated at
C        X and V. The i,j-th component of the array will contain the
C        derivative with respect to variables X(i) and X(j).
C
      ...
      IF ( NUMCON .GT. 0 ) THEN
         IF ( JTRANS ) THEN
            IF ( LCJAC1 .LT. N .OR. LCJAC2 .LT. M ) THEN
               IF ( LCJAC1 .LT. N ) WRITE( IOUT, 2020 )
               IF ( LCJAC2 .LT. M ) WRITE( IOUT, 2030 )
               STOP
            END IF
         ELSE
            IF ( LCJAC1 .LT. M .OR. LCJAC2 .LT. N ) THEN
               IF ( LCJAC1 .LT. M ) WRITE( IOUT, 2020 )
               IF ( LCJAC2 .LT. N ) WRITE( IOUT, 2030 )
               STOP
            END IF
         END IF
      END IF
...
C
      ...
      END IF
C
C  For unconstrained problems, skip construction of WK( LWKSTR + IG ) 
C  and skip specialized construction of gradient and Jacobian.  
C  Call ELGRD instead.
C
      IF ( NUMCON .GT. 0 ) THEN
C
...
C
      ...
      END IF
C
C  Initialize the dense Hessian matrix.
C
      DO 420 J = 1, N
         DO 410 I = 1, N
            H( I, J ) = ZERO
