...
CS    REAL              X( N ), V     ( LV ), CJAC  ( LCJAC )
CD    DOUBLE PRECISION  X( N ), V     ( LV ), CJAC  ( LCJAC )
C
C  Compute the gradients of the objective function and general
C  constraints of a function initially written in Standard
C  Input Format (SIF). The gradients are given in a sparse format.
C
