...
CS    REAL               X     ( N      ), G( N ), HI    ( LHI )
CD    DOUBLE PRECISION   X     ( N      ), G( N ), HI    ( LHI )
C
C  Compute the gradient and Hessian matrix of a group partially 
C  separable function initially written in Standard Input Format (SIF).
C
C  The Hessian matrix is represented in "finite element format", i.e., 
C
C           ne
C      H = sum H_i, 
C          i=1
