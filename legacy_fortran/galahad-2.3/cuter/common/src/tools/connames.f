C     ( Last modified on 23 Dec 2000 at 22:01:38 )
      ...
      INTEGER M
      CHARACTER * 10 GNAME( M )
C
C  Obtain the names of the general constraints.
C
C  D. Orban, for GOT productions, Aug 2005, from the version of
...
C
      INTEGER            I, IG
C
C  Set the names of the general constraints.
C
C                            only if there are constraints.
      IF ( NUMCON .GT. 0 ) THEN
         DO 20 IG = 1, NG
            IF ( IWK( KNDOFC + IG ) .NE. 0 )
