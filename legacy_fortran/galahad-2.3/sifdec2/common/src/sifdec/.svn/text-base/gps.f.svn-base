C  THIS VERSION: 13/04/2012 AT 13:45:00 GMT
C ** Correction report.
C ** Correction -6. 13/04/12: QMATRIX added as alias for QUADOBJ
...
C ** Correction 11. 18/10/06: 1 line added
      LOGICAL          AddDoLoop
C
C  DATA DECLARATIONS.
C
...
C  RECORD THE LOCATION OF THE DO-LOOP VARIABLE IN THE ARRAY INLIST.
C
C ** Correction 11. 18/10/06: In IF ( A .OR. B ), the Fortran Standard
C    may evaluate A and B in any order and may evaluate B even if A is
C    .TRUE. This causes a failure on some compilers since
C    NINSTR(LEVEL-1) was accessed even when LEVEL=1. Thanks to
C    Jeff Renfro for pointing this out.
         AddDoLoop = .FALSE.
         IF ( LEVEL .EQ. 1 ) THEN
...
C         IF ( ( LEVEL .EQ. 1 .AND.  FIELD2( 1: 10 ) .EQ.
C     *        NAMIIN( LOOP( 1 ) ) ) .OR.( LEVEL .GT. 1 .AND.
C     *        FIELD2( 1: 10 ) .EQ. NAMIIN(
C     *        INSTR( 2, NINSTR( LEVEL - 1 ), LEVEL - 1 ) ) ) ) THEN
