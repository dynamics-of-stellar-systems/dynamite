...
!-*-*-  L A N C E L O T  -B-   CAUCHY_get_exact_gcp  S U B R O U T I N E  -*-*
     ...
     SUBROUTINE CAUCHY_get_exact_gcp( n , X0, XT, G , BND   ,                  &
                                     VARIABLE_STATUS, f    , tmax  ,           &
                                     epstol, boundx, dxsqr , r, qxt,           &
                                     P , Q , IVAR  , nfree , nvar1 ,           &
                                     nvar2 , nnonnz, INONNZ, iout  ,           &
                                     jumpto, idebug, findmx, BREAKP, S )
...
!  ------------------ end of dummy arguments --------------------------
     ...
     REAL ( KIND = wp ), INTENT( IN ):: r, tmax, findmx
     REAL ( KIND = wp ), INTENT( INOUT ):: f, epstol, dxsqr, qxt
...
!  Non-executable statements
 ...
 2030  FORMAT( /, ' Piece', I5, ' - f, G and H at start point ', 4ES12.4 )
...
!-*-*-  L A N C E L O T  -B-  CAUCHY_get_approx_gcp  S U B R O U T I N E  -*-*
     ...
     SUBROUTINE CAUCHY_get_approx_gcp( n , X0, XT, G , BND   ,                 &
                                      VARIABLE_STATUS       , f     ,          &
                                      epstol, tmax  , rmu   , boundx,          &
                                      r, qxt, P , Q , IVAR  , nfree ,          &
                                      nvar1 , nvar2 , iout  , jumpto,          &
...
!  ------------------ end of dummy arguments --------------------------
     ...
     REAL ( KIND = wp ), INTENT( IN    ):: tmax, r, rmu, findmx
     REAL ( KIND = wp ), INTENT( INOUT ):: f, epstol, qxt
     LOGICAL, INTENT( IN ) :: boundx
     INTEGER, DIMENSION( n ), INTENT( INOUT ) :: VARIABLE_STATUS
...
!  Move the fixed variables to their bounds
         ...
         IF ( P( i ) /= zero ) XT( i ) = BND( i, VARIABLE_STATUS( i ) )
       END IF
     END DO
