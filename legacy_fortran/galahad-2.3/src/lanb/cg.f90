...
!-*-*-*-*-  L A N C E L O T  -B-    CG_solve   S U B R O U T I N E   -*-*-*-*
      ...
      SUBROUTINE CG_solve( n , X0, XT, G , BND, nbnd, VARIABLE_STATUS,         &
                           epsgrd, fmodel, XSCALE, GRAD  , status,             &
                           P , Q , IVAR  , nvar  , nresrt, twonrm, r,          &
                           nobnds, gnrmsq, dxsqr , iout  , jumpto, idebug,     &
                           findmx, itercg, itcgmx, ifixed, DIST_bound, S )
...
!  ------------------ end of dummy arguments --------------------------
      ...
      REAL ( KIND = wp ), INTENT( IN    ), DIMENSION( n ) :: XSCALE
