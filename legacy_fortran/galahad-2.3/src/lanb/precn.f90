...
!-*-  L A N C E L O T  -B-  PRECN_use_preconditioner S U B R O U T I N E -*-*
     ...
     SUBROUTINE PRECN_use_preconditioner(                                      &
         ...
         H_col_ptr, L_col_ptr, W, W1, RHS, RHS2, P2, G,                        &
         ISTAGV, ISVGRP,                                                       &
...
!--------------------------------------------------------------
     ...
     INTEGER, INTENT( INOUT ) :: llink, lpos
...
!  Allocate space to hold the band matrix
         ...
         CALL ASMBL_assemble_hessian(                                          &
             ...
             nvargp, nvar  , IVAR  , ISTADH, ICNA  ,                           &
             ISTADA, INTVAR, IELVAR, IELING, ISTADG, ISTAEV,                   &
             ISTAGV, ISVGRP, A     , GUVALS, lnguvl, HUVALS,                   &
             lnhuvl, GVALS2, GVALS3, GSCALE, ESCALE, GXEQX ,                   &
             ITYPEE, INTREP, RANGE , iprint, error, out, buffer, use_band,     &
             .TRUE., .FALSE. , S%nsemiw, status, alloc_status, bad_alloc,      &
             SA, lirnh, ljcnh, lh, matrix%row, matrix%col, matrix%val,         &
             LINK_col, POS_in_H, llink, lpos,                                  &
             IW_asmbl, W_ws, W_el, W_in, H_el, H_in, skipg,                    &
             maxsbw = maxsbw, DIAG = DIAG, OFFDIA = OFFDIA,                    &
             KNDOFG = KNDOFG )        
       ELSE
...
!  Set starting addresses for partitions of the integer workspace
         ...
         CALL ASMBL_assemble_hessian(                                          &
             ...
             SA, lirnh, ljcnh, lh, matrix%row, matrix%col, matrix%val,         &
             LINK_col, POS_in_H, llink, lpos,                                  &
             IW_asmbl, W_ws, W_el, W_in, H_el, H_in, skipg, nnzh = nnzh,       &
             KNDOFG = KNDOFG )        
       END IF
