...
!-------------------------------------------------
      ...
      TYPE, PUBLIC :: LCF_control_type
        ...
        REAL ( KIND = wp ) :: identical_bounds_tol, cpu_time_limit
      ...
      TYPE, PUBLIC :: LCF_inform_type
        ...
        INTEGER :: iterp1, iterp01, iterp001, iterp0001
...
!-*-*-*-*-*-   L C F _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-*
      ...
      SUBROUTINE LCF_initialize( data, control )
...
! =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ...
      TYPE ( LCF_data_type ), INTENT( OUT ) :: data
...
!     data%CNTL%thresh = 50
      CALL SBLS_initialize( data = data%SBLS_data,                             &
                            control = control%SBLS_control )
...
!  Local variables
      ...
      CHARACTER( LEN = 16 ), PARAMETER :: specname = 'LCF            '
...
!  =================
      ...
      CALL LCF_solve_main( data%dims, prob%n, prob%m,                          &
                           ...
                           data%C, data%H_sub, data%A_sub, data%C_sub,         &
                           data%SBLS_data, time_start,                         &
                           control, inform )
...
!-*-*-*-*-*-   L C F _ S O L V E _ M A I N   S U B R O U T I N E   -*-*-*-*-*
      ...
      SUBROUTINE LCF_solve_main( dims, n, m, A_val, A_col, A_ptr,              &
                                 ...
                                 SOL, C, H_sub, A_sub, C_sub,                  &
                                 SBLS_data, time_start, control, inform )
...
!  pointers (which will be allocatable arrays in f2000!!)
      ...
      TYPE ( SMT_type ), INTENT( INOUT ) :: H_sub, A_sub, C_sub
...
!  Automatic arrays
      ...
      REAL ( KIND = wp ),                                                      &
        DIMENSION( dims%c_l_start : dims%c_u_end ) :: Pb_c, Pl_c, C_eq, Z_c, PZ_c
...
!  and that the upper bounds are larger than the corresponing lower bounds
      ...
      DO i = dims%x_u_start, dims%x_l_end
        IF ( X_u( i ) - X_l( i ) <= epsmch ) THEN 
          inform%status = - 5 ; GO TO 600 ; END IF
      END DO
...
!  Provide storage for the 1,2 block, A augmented by its slack variables
      ...
      A_sub%m = m
      A_sub%n = ns
      A_sub%ne = A_ne + n_slack
      CALL SMT_put( A_sub%type, 'COORDINATE', inform%alloc_status )

      array_name = 'wcp: data%A_sub%row'
      CALL SPACE_resize_array( A_sub%ne, A_sub%row, inform%status,             &
             inform%alloc_status, array_name = array_name,                     &
             deallocate_error_fatal = control%deallocate_error_fatal,          &
             exact_size = control%space_critical,                              &
             bad_alloc = inform%bad_alloc, out = control%error )
      ...
      array_name = 'wcp: data%A_sub%col'
      CALL SPACE_resize_array( A_sub%ne, A_sub%col, inform%status,             &
             inform%alloc_status, array_name = array_name,                     &
             deallocate_error_fatal = control%deallocate_error_fatal,          &
             exact_size = control%space_critical,                              &
             bad_alloc = inform%bad_alloc, out = control%error )
      ...
      array_name = 'wcp: data%A_sub%val'
      CALL SPACE_resize_array( A_sub%ne, A_sub%val, inform%status,             &
             inform%alloc_status, array_name = array_name,                     &
             deallocate_error_fatal = control%deallocate_error_fatal,          &
             exact_size = control%space_critical,                              &
             bad_alloc = inform%bad_alloc, out = control%error )
...
!  Insert A into A_sub
      ...
      DO i = 1, m
        A_sub%row( A_ptr( i ) : A_ptr( i + 1 ) - 1 ) = i
      END DO
      A_sub%col( : A_ne ) = A_col( : A_ne ) 
      A_sub%val( : A_ne ) = A_val( : A_ne )
      j = 0
      DO i = dims%c_l_start, dims%c_u_end
        j = j + 1
        A_sub%row( A_ne + j ) = i
        A_sub%col( A_ne + j ) = n + j
        A_sub%val( A_ne + j ) = - one
      END DO

!  provide storage for the identity 1,1 block

      H_sub%n = ns
      array_name = 'wcp: data%H_sub%val'
      CALL SPACE_resize_array( H_sub%n, H_sub%val, inform%status,              &
             inform%alloc_status, array_name = array_name,                     &
             deallocate_error_fatal = control%deallocate_error_fatal,          &
             exact_size = control%space_critical,                              &
             bad_alloc = inform%bad_alloc, out = control%error )
      ...
      CALL SMT_put( H_sub%type, 'DIAGONAL', inform%alloc_status )
      H_sub%val = one

!  provide storage for the zero 2,2 block

      CALL SMT_put( C_sub%type, 'ZERO', inform%alloc_status )
...
!      ( A  -C  )   ( A   0  )
      ...
      CALL SBLS_form_and_factorize( ns, m, H_sub, A_sub, C_sub,                &
...
!  Compute the orthogonal projection Pb of (x,c) into their bounds
          ...
          IF ( control%step_strategy == 0 .OR. inform%iter == 0 ) THEN
...
!  project onto linear constraints
            ...
            CALL SBLS_solve( ns, m, A_sub, C_sub, SBLS_data,                   &
          ...
          IF ( MOD( inform%iter - MAX( 0, start_print ), print_gap ) == 0 ) THEN
           ...
           &     '      size_l   #infeas  l_b  alp      time' )" )
            ...
            IF ( printi ) WRITE( out, "( I8, 3ES12.4, I8, 2F5.2, F10.2 )" )    &
              ...
              inform%num_infeas, lambda_b, alpha, time
          END IF
...
!  Compute the new point
          ...
          IF ( control%step_strategy > 0 ) THEN
...
!  Project onto the feasible region for the bounds
          ...
          IF ( control%project_onto_bounds_first .OR. inform%iter > 1 ) THEN
            Pb_x = MIN( X_u, MAX( X_l, X ) )
            Pb_c = MIN( C_u( dims%c_l_start : dims%c_u_end ),                  &
                     MAX( C_l( dims%c_l_start : dims%c_u_end ), C ) )
...
!  Compute the residual for the equality constraints
            ...
            CALL LCF_AX( m, C_RES( : m ), m, A_ne, A_val, A_col, A_ptr, n,     &
                         X, '+ ' )
            inform%size_l = MAX( MAXVAL( ABS( C_RES( : dims%c_equality ) -     &
                                       C_l( : dims%c_equality ) ) ),           &
...
!  Compute the violation of the bounds
          ...
          IF ( MOD( inform%iter - MAX( 0, start_print ), print_gap ) == 0 ) THEN
            IF ( printt )                                                      &
             ...
             &    '  #infeas  alp      time')")
            IF ( printi ) THEN
              WRITE( out, "( I8, 3ES12.4, I8, F5.2, F10.2 )" ) inform%iter,    &
...
!  Deallocate all remaining allocated arrays
      ...
      array_name = 'lcf: data%A_sub%row'
      CALL SPACE_dealloc_array( data%A_sub%row,                                &
         inform%status, inform%alloc_status, array_name = array_name,          &
         bad_alloc = inform%bad_alloc, out = control%error )
      IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

      array_name = 'lcf: data%A_sub%col'
      CALL SPACE_dealloc_array( data%A_sub%col,                                &
         inform%status, inform%alloc_status, array_name = array_name,          &
         bad_alloc = inform%bad_alloc, out = control%error )
      IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

      array_name = 'lcf: data%A_sub%val'
      CALL SPACE_dealloc_array( data%A_sub%val,                                &
         inform%status, inform%alloc_status, array_name = array_name,          &
         bad_alloc = inform%bad_alloc, out = control%error )
      IF ( control%deallocate_error_fatal .AND. inform%status /= 0 ) RETURN

      array_name = 'lcf: data%H_sub%val'
