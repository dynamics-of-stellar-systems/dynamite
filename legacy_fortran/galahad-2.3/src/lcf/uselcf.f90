...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
   ...
   MODULE GALAHAD_USELCF_double

!    ------------------------------------------------------------
!    | CUTEr/AMPL interface to LCF, an alternating projection   |
...
!-*-*-*-*-*-*-*-*-*-   U S E _ L C F  S U B R O U T I N E   -*-*-*-*-*-*-*-*-

     SUBROUTINE USE_LCF( input )
...
!         x_l <=  x <= x_u
!
!  specified by CUTEr using the GALAHAD package GALAHAD_LCF
!
!  --------------------------------------------------------
...
!  Scalars
      ...
      INTEGER :: n, m, ir, ic, ifail, nmax, mmax, la, liw, iores
!     INTEGER :: np1, npm
      INTEGER :: i, j, l, nea, factorization_integer, factorization_real
...
!  Specfile characteristics
      ...
      CHARACTER ( LEN = 16 ) :: specname = 'RUNLCF'
      TYPE ( SPECFILE_item_type ), DIMENSION( lspec ) :: spec
      CHARACTER ( LEN = 16 ) :: runspec = 'RUNLCF.SPC'

!  The default values for LCF could have been set as:

! BEGIN RUNLCF SPECIFICATIONS (DEFAULT)
...
!  Arrays
      ...
      TYPE ( QTRANS_trans_type ) :: QTRANS_trans
      TYPE ( QTRANS_data_type ) :: QTRANS_data
      TYPE ( QTRANS_control_type ) :: QTRANS_control
      TYPE ( QTRANS_inform_type ) :: QTRANS_inform

!  Allocatable arrays

      CHARACTER ( LEN = 10 ), ALLOCATABLE, DIMENSION( : ) :: VNAME, CNAME
      REAL ( KIND = wp ), ALLOCATABLE, DIMENSION( : ) :: SH, SA, C
...
!  functions (C)
      ...
      CALL CFN( n, m, prob%X0, objf, m, C( : m ) )
      DO i = 1, m 
        IF ( EQUATN( i ) ) THEN 
...
!  Evaluate the linear terms of the constraint functions
      ...
      CALL CSGR( n, m, .FALSE., mmax, prob%Y, prob%X0, nea, la, prob%A%val,    &
...
!  Set all default values, and override defaults if requested
      ...
      CALL LCF_initialize( data, LCF_control )
      IF ( is_specfile )                                                       &
...
!  If required, scale the problem
      ...
      IF ( scale < 0 ) THEN
        ALLOCATE( SH( prob%n ), SA( prob%m ), STAT = alloc_stat )
        IF ( alloc_stat /= 0 ) THEN
...
!  Scale using K
        ...
        IF ( scale == - 1 .OR. scale == - 4 ) THEN
          ...
          CALL SCALING_get_factors_from_K( prob%n, prob%m, prob%H%val,         &
                                           prob%H%col, prob%H%ptr,             &
                                           prob%A%val, prob%A%col,             &
                                           prob%A%ptr, SH, SA,                 &
                                           control, ifail )
!  Scale using A
        ...
        ELSE IF ( scale == - 2 .OR. scale == - 5 ) THEN
          ...
          CALL SCALING_get_factors_from_A( prob%n, prob%m, prob%A%val,         &
...
!  Rescale A
        ...
        IF ( scale <= - 3 .AND. scale >= - 5 ) THEN
          IF ( printo ) WRITE( out, 2170 )
          CALL SCALING_normalize_rows_of_A( prob%n, prob%m, prob%A%val,        &
                                            prob%A%col, prob%A%ptr, SH, SA )
        END IF
...
!  Apply the scaling factors
        ...
        IF ( scale >= - 5 ) THEN
          CALL SCALING_apply_factors( prob%n, prob%m, prob%H%val, prob%H%col,  &
                                      prob%H%ptr, prob%A%val, prob%A%col,      &
                                      prob%A%ptr, prob%G, prob%X, prob%X_l,    &
                                      prob%X_u, prob%C_l, prob%C_u, prob%Y,    &
                                      prob%Z, infinity, SH, SA, .TRUE. )
        ELSE

!  Try the new scaling package

!  Initialize data

          CALL QTRANS_initialize( QTRANS_control, QTRANS_inform )
          QTRANS_control%infinity = 1.0D+18
!         QTRANS_control%shift_f = 0
!         QTRANS_control%scale_f = 0
          QTRANS_control%scale_x_min = ten ** ( - 5 )
          QTRANS_control%scale_c_min = ten ** ( - 5 )

!  Get the scale factors

          CALL QTRANS_get_factors( QTRANS_trans, QTRANS_data, QTRANS_control,  &
                                   QTRANS_inform, prob%m, prob%n, prob%f,      &
                                   prob%X, prob%X_l, prob%X_u, prob%C,         &
                                   prob%C_l, prob%C_u, prob%G, prob%A%col,     &
                                   prob%A%ptr, prob%A%val ) 

!  Apply the scalings to the problem

          WRITE( out, "( ' max A before scaling ', /, ES12.4 )" )              &
            MAXVAL( ABS( prob%A%val( : prob%A%ptr( prob%m + 1 ) - 1 ) ) )
          CALL QTRANS_apply( QTRANS_trans, QTRANS_data, QTRANS_control,        &
                             QTRANS_inform, prob%m, prob%n, prob%f, prob%G,    &
                             prob%X, prob%X_l, prob%X_u, prob%C_l,             &
                             prob%C_u, prob%A%col, prob%A%ptr,                 &
                             prob%A%val, prob%H%col, prob%H%ptr, prob%H%val ) 
          WRITE( out, "( ' max A after scaling ', /, ES12.4 )" )               &
            MAXVAL( ABS( prob%A%val( : prob%A%ptr( prob%m + 1 ) - 1 ) ) )
        END IF
        IF ( write_scaled_sif ) THEN
...
!       Overide some defaults
        ...
        PRE_control%infinity   = LCF_control%infinity
        PRE_control%c_accuracy = ten * LCF_control%stop_p
        PRE_control%z_accuracy = ten * LCF_control%stop_d
...
!           QTRANS_control%shift_f = 0
!           QTRANS_control%scale_f = 0
!           QTRANS_control%scale_x_min = ten ** ( - 3 )
!           QTRANS_control%scale_c_min = ten ** ( - 3 )
...
!       WRITE( 33, "( ' y ', /, ( 5ES12.4 ) )" ) prob%Y( : prob%m )
!       WRITE( 33, "( ' z ', /, ( 5ES12.4 ) )" ) prob%Z( : prob%n )
...
!       prob%X( : prob%n ) = zero
        ...
        CALL LCF_solve( prob, data, LCF_control, LCF_inform )

        qfval = LCF_inform%obj

        CALL CPU_TIME( timet )
  
!  Deallocate arrays from the minimization
  
        status = LCF_inform%status ; iter = LCF_inform%iter
        nfacts = LCF_inform%nfacts ; stopr = LCF_control%stop_d
        factorization_integer = LCF_inform%factorization_integer 
        factorization_real = LCF_inform%factorization_real
        CALL LCF_terminate( data, LCF_control, LCF_inform )
        ...
        IF ( scale > 0 ) THEN
          IF ( scale <= 5 ) THEN
            CALL SCALING_apply_factors( prob%n, prob%m, prob%H%val, prob%H%col,&
                                        ...
                                        prob%A%ptr, prob%G, prob%X, prob%X_l,  &
                                        prob%X_u,  prob%C_l, prob%C_u, prob%Y, &
                                        prob%Z, infinity, SH, SA, .FALSE.,     &
                                        C = prob%C )
            DEALLOCATE( SH, SA )
          ELSE
            CALL QTRANS_apply_inverse( QTRANS_trans, QTRANS_data,              &
                                       QTRANS_control, QTRANS_inform, prob%m,  &
                                       prob%n, prob%f, prob%G, prob%X,         &
                                       prob%X_l, prob%X_u, prob%C, prob%C_l,   &
                                       prob%C_u, prob%A%col, prob%A%ptr,       &
                                       prob%A%val, prob%H%col, prob%H%ptr,     &
                                       prob%H%val ) 
          END IF
        END IF
      ELSE
        ...
        DO j = 1, 2 
          ...
          DO i = ir, ic 
            ...
            ELSE IF ( prob%X( i ) - prob%X_u( i ) > ten * stopr ) THEN
              state = 'ABOVE'
            ELSE IF ( ABS( prob%Z( i ) ) < ten * stopr ) THEN
              state = 'DFREE'
            END IF
          ...
          DO j = 1, 2 
            ...
            DO i = ir, ic 
              ...
              ELSE IF ( prob%C( I ) - prob%C_u( i ) > ten * stopr ) THEN
                state = 'ABOVE' 
              ELSE IF ( ABS( prob%Y( i ) ) < ten * stopr ) THEN
                state = 'DFREE'
              END IF
...
!  Compare the variants used so far
      ...
      IF ( write_result_summary ) THEN
        ...
        WRITE( rfiledevice, 2190 ) pname, n, m, iter,                          &
          LCF_inform%iterp1, LCF_inform%iterp01, LCF_inform%iterp001,          &
          LCF_inform%iterp0001, LCF_inform%size_l, LCF_inform%size_b,          &
          LCF_inform%num_infeas, status, timet,                                &
          LCF_control%use_simultaneous_projection, LCF_control%step_strategy
      END IF
...
!  Non-executable statements
 ...
 2060 FORMAT( ' Total time = ', 0P, F0.2 ) 
 ...
 2180 FORMAT( A10 )
 2190 FORMAT( A10, 2I7, 5I9, 2ES10.2, 2I7, 0P, F8.2, L2, I0 ) 
...
!  End of subroutine USE_LCF

     END SUBROUTINE USE_LCF

!  End of module USELCF_double

   END MODULE GALAHAD_USELCF_double
