...
!    Restore the data.
     ...
     problem%x_u      = (/  2.0D0,  2.0D0 /)
     IF ( .NOT. ASSOCIATED ( problem%c )   ) ALLOCATE( problem%c( problem%m ))
     ...
     IF ( .NOT. ASSOCIATED ( problem%g )    ) ALLOCATE( problem%g( problem%n ))
     IF ( .NOT. ASSOCIATED ( problem%J_val ) ) ALLOCATE( problem%J_val( J_size))
