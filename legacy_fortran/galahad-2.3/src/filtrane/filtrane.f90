...
!*****************************************************
      ...
      CASE ( GET_JTC )
         ...
         RETURN
      END SELECT
      s%g_norm2     = TWO_NORM( problem%n, problem%g, 1 )
...
!  Compute the norm of the current iterate.
     ...
     s%x_norm = TWO_NORM( problem%n, problem%x, 1 )
...
!             ......................
              ...
              ELSE IF ( s%GLTR_info%status == GALAHAD_warning_on_boundary .OR. &
                 ...
                 ELSE
                    s%it_status( 3:3 ) = 'E'
                 END IF
                 s%s_norm2 = TWO_NORM( problem%n, s%step, 1 )
...
!     Compute the norm of the current theta.
      ...
      normtheta = TWO_NORM( s%p, s%theta, 1 )
...
!     Compute the margin, when it only depends on the current theta.
      ...
      cnorm = TWO_NORM( s%p, s%theta( 1:s%p ), 1 )
