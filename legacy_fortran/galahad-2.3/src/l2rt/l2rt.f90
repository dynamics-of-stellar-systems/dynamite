! THIS VERSION: GALAHAD 2.2 - 30/05/2008 AT 15:00 GMT.
...
!      |                                                     |
!      |    minimize  sqrt(|| A x - b ||^2 + mu ||p||^2)     |
!      |               + 1/p sigma || x ||^p                 |
!      |                                                     |
!      | for given sigma, mu >= 0 and and p >= 2 using a     |
...
!-*-*-*-*-*-  L 2 R T _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-*-
      ...
      SUBROUTINE L2RT_initialize( data, control )
...
!-----------------------------------------------
      ...
      TYPE ( L2RT_DATA_TYPE ), INTENT( OUT ) :: data
...
!  Local variables
      ...
      CHARACTER( LEN = 16 ), PARAMETER :: specname = 'L2RT            '
...
!-*-*-*-*-*-*-*-*-*-*  L 2 R T _ S O L V E   S U B R O U T I N E  -*-*-*-*-*-*-*

      SUBROUTINE L2RT_solve( m, n, p, sigma, mu, X, U, V, data, control, inform )
...
!   data     private internal data
!   control  a structure containing control information. See L2RT_initialize
...
!                   mu + sigma * ||x||^(p-2) * sqrt(||Ax-b||^2 + mu ||x||^2)
...
!---------------------------------
      ...
      REAL ( KIND = wp ), INTENT( IN ) :: p, sigma, mu
...
!---------------------------------
      ...
      INTEGER :: i
      REAL ( KIND = wp ) :: alpha_1, dec_tol, rho_tilde, phi_tilde
...
!  Check for obvious errors
      ...
      IF ( mu < zero ) GO TO 970
      IF ( p < two ) GO TO 980
      data%one_pass = sigma == zero .AND. mu == zero
...
!  Allocate worspace for the bi-diagonal solves
        ...
        array_name = 'gltr: RES'
        CALL SPACE_resize_array( data%itmax + 1, data%RES,                      &
            inform%status, inform%alloc_status, array_name = array_name,        &
            deallocate_error_fatal = control%deallocate_error_fatal,            &
            exact_size = control%space_critical,                                &
            bad_alloc = inform%bad_alloc, out = control%error )
...
!  Special case when sigma and mu are both zero
...
!  Initialize ||r_1|| and ||A^T r_1|| , where the residual r_1 = b - A x_1
      ...
      data%obj_0 = inform%r_norm ; inform%obj = data%obj_0
...
!  Find the solution on the boundary in the subspace x = V_k y
          ...
          IF ( data%try_warm ) THEN
            IF ( inform%iter > 2 ) THEN
              IF ( data%LAMBDA( inform%iter - 1 ) >                             &
                   data%LAMBDA( inform%iter - 2 ) ) THEN
                data%LAMBDA( inform%iter ) = data%LAMBDA( inform%iter - 1 )
              ELSE
!               data%LAMBDA( inform%iter ) =                                    &
!                 0.9_wp * data%LAMBDA( inform%iter - 1 )
                data%LAMBDA( inform%iter ) = data%LAMBDA( inform%iter - 1 )
              END IF
            ELSE
              data%LAMBDA( inform%iter ) = data%LAMBDA( inform%iter - 1 )
            END IF
          END IF
          CALL L2RT_solve_bidiagonal( inform%iter,                              &
            data%B_diag( : inform%iter ), data%B_offdiag( : inform%iter ),      &
            data%beta_1, p, sigma, mu, data%LAMBDA( inform%iter ),              &
            data%Y( : inform%iter ), data%H( : inform%iter ), inform%x_norm,    &
            data%RES( : inform%iter + 1 ),                                      &
...
!  Compute the norms ||r_k|| and ||A^T r_k||
          ...
          inform%ATr_norm = ABS( data%alpha_kp1 * data%B_offdiag( inform%iter ) &
                                 * data%Y( inform%iter ) )
          inform%obj = SQRT( inform%r_norm ** 2 + mu * inform%x_norm ** 2 ) +   &
...
!  Print progress
          ...
          IF ( data%printi ) THEN
            ...
            WRITE( control%out, "( A, I7, 2ES16.8, 3ES9.2, I7 )" ) prefix,      &
               inform%iter, inform%obj, inform%r_norm, inform%ATr_norm,         &
               inform%x_norm, data%LAMBDA( inform%iter ), data%biter
          END IF
...
!  Recover the solution on the boundary in the subspace x = V_pass2 y
      ...
      CALL L2RT_solve_bidiagonal( data%end_pass2,                               &
        data%B_diag( : data%end_pass2 ),                                        &
        data%B_offdiag( : data%end_pass2 ),                                     &
        data%beta_1, p, sigma, mu, data%LAMBDA( data%end_pass2 ),               &
        data%Y( : data%end_pass2 ), data%H( : data%end_pass2 ), inform%x_norm,  &
        data%RES( : data%end_pass2 + 1 ), data%R_diag( : data%end_pass2 ),      &
...
!  Regularisation order to small
  ...
  970 CONTINUE
      IF ( control%error > 0 .AND. control%print_level > 0 )                    &
        WRITE( control%error,                                                   &
         "( A, ' s ', ES12.4 , ' is negative ' )" ) prefix, mu
      inform%status = GALAHAD_error_restrictions
      RETURN
...
!  End of subroutine L2RT_solve

      END SUBROUTINE L2RT_solve

!-*-*-*-*-*-  L 2 R T _ T E R M I N A T E   S U B R O U T I N E   -*-*-*-*-*-*-

      SUBROUTINE L2RT_terminate( data, control, inform )

!  ..............................................
!  .                                            .
!  .  Deallocate arrays at end of L2RT_solve    .
!  .                                            .
!  ..............................................
...
!
!   data    private internal data
!   control see Subroutine L2RT_initialize
!   inform    see Subroutine L2RT_solve
...
!  End of subroutine L2RT_terminate
...
!-*-*-*-  L 2 R T _ S O L V E _ B I D I A G O N A L   S U B R O U T I N E  *-*-*-
      ...
      SUBROUTINE L2RT_solve_bidiagonal( n, B_diag, B_offdiag, beta, p, sigma,   &
                                        mu, lambda, Y, H, y_norm, RES, R_diag,  &
                                        R_offdiag, F, G, error_tol, try_warm,   &
                                        print_level, out, prefix, itmax, iter,  &
                                        status )

! ---------------------------------------------------------------------------
!
!  Solve min  sqrt(|| B_k y - beta_1 e_1 ||^2 + mu ||y||^2) + 1/p sigma ||y||^p
!
!  where
!           (  diag_1                              )
!           ( offdiag_1  diag_2                    )
...
!-----------------------------------------------
      ...
      REAL ( KIND = wp ), INTENT( OUT ), DIMENSION( n + 1 ) :: RES
...
!-----------------------------------------------
      ...
      REAL ( KIND = wp ) :: h_norm, error, power
      REAL ( KIND = wp ) :: phi, phi_prime, psi, psi_prime, dl_phi, dl_psi
      REAL ( KIND = wp ) :: pi, pi_prime, nu, nu_prime, omega, prod
...
!  Compute the solution y(lambda) to the bi-diagonal subproblem
        ...
        CALL L2RT_backsolve_bidiagonal( n, R_diag, R_offdiag, F, Y, .FALSE. )

!  Compute the residual B y(lambda) - beta e_1

        RES( 1 : n ) = B_diag * Y
        RES( n + 1 ) = zero
        RES( 1 ) = RES( 1 ) - beta
        RES( 2 : n + 1 ) = RES( 2 : n + 1 ) + B_offdiag * Y

!  Compute pi = || y(lambda) ||
!  and nu = sqrt( || B y(lambda) - beta e_1 ||^2 + mu || y(lambda) ||^2 )
...
!  Compute the solution R^T(lambda) v = y
        ...
        CALL L2RT_backsolve_bidiagonal( n, R_diag, R_offdiag, Y, H, .TRUE. )
        h_norm = TWO_NORM( H )
...
!  Alternatives are Newton corrections to the functions given in the TRR paper

          IF ( printd ) THEN
            IF ( p > two ) THEN

!  Compute psi and its Newton correction

              prod = nu / lambda
              psi = prod ** power - one / ( sigma * omega ) ** power
              psi_prime = power * ( prod ** ( power - one ) *                   &
                  ( nu_prime / lambda - nu /lambda ** 2 )                       &
...
!  Compute the Newton update
          ...
          lambda = lambda + dl_phi

!  The warm start failed - revert to a cautious starting guess

        ELSE
          lambda = MAX( mu + lambda_start, lambda + dl_phi )
        END IF
...
!  Test for exceptional convergence ( B y = beta e_1 ). Set lambda to zero
...
!  Non-executable statement
 ...
 2000 FORMAT( /, A, '   Iter          phi            lambda       ||y||' )
 2010 FORMAT( A, ' corrections: phi ', ES16.8, ' psi ', ES16.8 )
 2020 FORMAT( A, ' correction: phi ', ES16.8 )
