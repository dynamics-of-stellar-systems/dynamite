! THIS VERSION: GALAHAD 2.1 - 22/03/2007 AT 09:00 GMT.
   ...
   INTEGER :: data_storage_type, i, status, iname = 0
   CHARACTER ( len = 1 ) :: st
   CHARACTER ( len = 10 ) :: sname

   n = 3 ; m = 2 ; h_ne = 4 ; a_ne = 4 
   ALLOCATE( p%G( n ), p%X_l( n ), p%X_u( n ), p%C( m ), p%C_l( m ), p%C_u( m ) )
...
!  test with new and existing data
     ...
     CALL QPP_terminate( map, control, info )
     DEALLOCATE( p%A%val, p%A%row, p%A%col )
     IF ( data_storage_type <= 0 ) DEALLOCATE( p%H%val, p%H%row, p%H%col )
     IF ( data_storage_type > 0 )  DEALLOCATE( p%X0 )
     IF ( data_storage_type > 1 )  DEALLOCATE( p%WEIGHT )
     IF ( data_storage_type <= 2 .AND.                                         &
          data_storage_type >= - 2 ) DEALLOCATE( p%G )
   END DO
