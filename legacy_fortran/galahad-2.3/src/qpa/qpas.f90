...
! problem data complete   
   CALL QPA_initialize( data, control )       ! Initialize control parameters
   control%infinity = infinity                ! Set infinity
   control%solve_qp = .TRUE.
   CALL QPA_solve( p, C_stat, B_stat, data, control, info )   ! Solve problem
   IF ( info%status == 0 ) THEN               !  Successful return
     WRITE( 6, "( ' QPA: ', I0, ' iterations. Optimal objective value =',      &
    &       ES12.4, /, ' Optimal solution = ', ( 5ES12.4 ) )" )                &
     info%iter, info%obj, p%X
   ELSE                                       !  Error returns
     WRITE( 6, "( ' QPA_solve exit status = ', I6 ) " ) info%status
   END IF
   CALL QPA_terminate( data, control, info )  !  delete internal workspace
