! THIS VERSION: GALAHAD 2.2 - 22/04/2008 AT 15:00 GMT.
...
!NOT95USE GALAHAD_CPU_time
      ...
      USE GALAHAD_TOOLS_double, ONLY: TOOLS_is_are, TOOLS_s
...
!--------------------
      ...
      TYPE, PUBLIC :: QPA_partition_type
        INTEGER :: n_free, n_fixed, c_ref, m_ref, k_ref, n_ref
        INTEGER :: k_free_od, k_free_d, k_free_p, k_fixed_od, k_fixed_d
      END TYPE
      ...
      TYPE, PUBLIC :: QPA_control_type
        ...
        INTEGER :: factor, max_col, max_sc, indmin, valmin, itref_max
        INTEGER :: infeas_check_interval, cg_maxit, precon, nsemib
        INTEGER :: full_max_fill, deletion_strategy
        INTEGER :: restore_problem, monitor_residuals, cold_start
        REAL ( KIND = wp ) :: infinity, feas_tol, obj_unbounded
        REAL ( KIND = wp ) :: increase_rho_g_factor, infeas_g_improved_by_factor
        REAL ( KIND = wp ) :: increase_rho_b_factor, infeas_b_improved_by_factor
        REAL ( KIND = wp ) :: pivot_tol, pivot_tol_for_dependencies, zero_pivot
        REAL ( KIND = wp ) :: inner_stop_relative, inner_stop_absolute
        REAL ( KIND = wp ) :: multiplier_tol, cpu_time_limit
        LOGICAL :: treat_zero_bounds_as_general, solve_qp, solve_within_bounds
        LOGICAL :: randomize, array_syntax_worse_than_do_loop
        CHARACTER ( LEN = 30 ) :: prefix
! components specifically for parametric problems
        LOGICAL :: each_interval
      END TYPE

      TYPE, PUBLIC :: QPA_inform_type
        INTEGER :: status, alloc_status, major_iter, iter, cg_iter
        INTEGER :: nfacts, nmods, factorization_integer, factorization_real
        INTEGER :: factorization_status, num_g_infeas, num_b_infeas
        REAL ( KIND = wp ) :: obj, infeas_g, infeas_b, merit
...
!----------------------
      ...
      INTEGER, PARAMETER :: SILS_flags = 8
      INTEGER, PARAMETER :: QPA_flags = 6
      INTEGER, PARAMETER :: flags = QPA_flags + SILS_flags
...
!-*-*-*-*-*-   Q P A _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-
      ...
      SUBROUTINE QPA_initialize( data, control )
...
!  Dummy arguments
      ...
      TYPE ( QPA_data_type ), INTENT( OUT ) :: data
...
!     control%pivot_tol = data%CNTLA%u
...
!
!  On exit from QPA_solve, other components of inform give the 
!  following:
!
...
!       solution determined by QPA_solve.
...
!  Allocate workspace arrays
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%IBREAK' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%RES_l' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%RES_u( data%dims%c_u_start : prob%m ),                  &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%RES_u' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%A_norms' ; GO TO 900
        END IF
      END IF
...
!  ========================================
      ...
      CALL QPA_remove_dependent( prob%n, prob%m, prob%A%val, prob%A%col,       &
                                 ...
                                 data%CNTLA, C_stat, B_stat, data%IBREAK,      &
                                 control, inform, n_depen )
      CALL CPU_TIME( dum ) ; dum = dum - time
      inform%time%preprocess = inform%time%preprocess + dum
...
!  Allocate more real workspace arrays
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%H_s' ; GO TO 900
        END IF
      END IF
...
!  Allocate real workspace
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%BREAKP' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%A_s' ; GO TO 900
        END IF
      END IF
      ...
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( data%PERT( prob%m + prob%n ),                      &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%PERT' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%GRAD' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%VECTOR' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%RHS( k_n_max + control%max_sc ),                        &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%RHS' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%S( k_n_max + control%max_sc ),                          &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%S' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%B( k_n_max + control%max_sc ),                          &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%B' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%RES( k_n_max + control%max_sc ),                        &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%RES' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%S_perm( k_n_max + control%max_sc ),                     &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%S_perm' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%DX( k_n_max + control%max_sc ),                         &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%DX' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%RES_print( k_n_max + control%max_sc ),                  &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%RES_print' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%R_pcg' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%X_pcg' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%P_pcg' ; GO TO 900
        END IF
      END IF
...
!  Allocate integer workspace arrays
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%SC' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
       ALLOCATE( data%REF( m_link ), STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%REF' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%C_up_or_low( data%dims%c_u_start : data%dims%c_l_end ), &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%C_up_or_low' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%X_up_or_low( data%dims%x_u_start : data%dims%x_l_end ), &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%X_up_or_low' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%PERM( k_n_max + control%max_sc ),                      &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%PERM' ; GO TO 900
        END IF
      END IF
...
!  Allocate arrays
      ...
      IF ( reallocate ) THEN 
        ALLOCATE( data%SCU_mat%BD_col_start( control%max_sc + 1 ),            &
                  STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_col_start' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_val' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_row' ; GO TO 900
        END IF
      END IF
      ...
      IF ( reallocate ) THEN 
        ...
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: data%DIAG' ; GO TO 900
        END IF
      END IF
...
!  Non-executable statements
 ...
 2000 FORMAT( /, 14X, ' =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=',            &
              ...
              /, 14X, ' =', 3F11.2, 6x, '=',                                   &
              /, 14X, ' =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=' )
 2010 FORMAT( ' ', /, '   **  Error return ',I3,' from QPA ' ) 
 2040 FORMAT( '   **  Error return ', I6, ' from ', A20 ) 
 2240 FORMAT( /, '  Warning - an entry from strict upper triangle of H given ' )
 2900 FORMAT( ' ** Message from -QPA_solve-', /,                              &
...
! -*-*-*-*-*-*-*-   Q P A _ S O L V E _ Q P   S U B R O U T I N E   -*-*-*-*-*-*-
      ...
      SUBROUTINE QPA_solve_qp( dims, n, m, H_val, H_col, H_ptr, G, f, rho_g,   &
                               ...
                               PERM, FACTORS, CNTLA, AINFO, FINFO,             &
                               SCU_mat, SCU_info, SCU_data, K,                 &
                               seed, time_inner_start,                         &
                               start_print, stop_print, prec_hist,             &
                               auto_prec, auto_fact, printi, control, inform,  &
                               G_p, X_p, Y_p, Z_p )
...
! *** nb change control back to "in" here and before
      ...
      TYPE ( SILS_control ), INTENT( INOUT ) :: CNTLA
...
!  =============
      ...
      DO
        ...
        CALL QPA_solve_main( dims, n, m, H_val, H_col, H_ptr, G, f, rho_g,      &
                       ...
                       X_up_or_low, PERM, FACTORS, CNTLA, AINFO, FINFO,         &
                       SCU_mat, SCU_info, SCU_data, K, warm_start,              &
                       time_inner_start, last_infeas_g, last_infeas_b,          &
                       start_print, stop_print, prec_hist, auto_prec,           &
                       auto_fact, pass, feas_tol, control, inform,              &
...
!  Tighten the infeasibility tolerance
            ...
            IF ( printi ) WRITE( control%out,                                  &
                 "( /, 16X, ' tightening infeasibility tolerance ...')" )
            got_sol = .TRUE.
          ELSE
...
!  Check to see that the infeasibility is decreasing
            ...
            IF ( control%solve_qp ) THEN
              IF ( inform%infeas_g + inform%infeas_b                           &
                ...
                icrit = icrit + 1
                IF ( icrit >= ncrit ) THEN
                  IF ( printi ) WRITE( control%out, 2160 ) ncrit
...
!  Check to see that the infeasibility is decreasing
          ...
          END IF
        END IF
        IF ( printi ) WRITE( control%out, "( /, 16X, ' infeasibility =',       &
       &    ES11.4, ', restarting ' )" ) inform%infeas_g + inform%infeas_b
        warm_start = .TRUE.
      END DO
...
!  Non-executable statements
 ...
 2160 FORMAT( /, ' Constraint violations have not decreased',                  &
                 ' substantially over', I3, ' major iterations. ',             &
...
!-*-*-*-*-*-    Q P A _ S O L V E _ M A I N   S U B R O U T I N E   -*-*-*-*-
      ...
      SUBROUTINE QPA_solve_main( dims, n, m,                                  &
                                 ...
                                 PERM, FACTORS, CNTLA, AINFO, FINFO,          &
                                 SCU_mat, SCU_info, SCU_data, K, warm_start,  &
                                 time_start, best_infeas_g, best_infeas_b,    &
                                 start_print, stop_print, prec_hist,          &
                                 auto_prec, auto_fact, pass, feas_tol,        &
...
!
!     100 + I The auxillary routine SILS has failed with error flag
!        IFLAG = I. See the specification of SILS for further details
!  
...
!
!     iter   - The total number of iterations required.
!     major_iter - The total number of major iterations required.
!     SILSiw - The total integer workspace required by SILS for the 
!              factorization.
!     SILSrw - The total real workspace required by SILS for the factorization.
!     nfacts - The total number of factorizations performed.
!
! =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
!  Local variables
      ...
      INTEGER :: m_active, sinfo, linesearch_inform, j_min, itref_max, n_all
      ...
      IF ( control%out > 0 .AND. control%print_level >= 5 )                    &
       WRITE( control%out, "( ' entering QPA_solve_main ' )" )
...
!  Set automatic choices for the factorizations and preconditioner
        ...
        IF ( printi )                                                          &
          WRITE( out, "( /, 18X, ' ====================================== ',   &
       &                 /, 18X, '      Start of Major iteration', I5,         &
       &                 /, 18X, '               (pass', I5, ')',              &
       &                 /, 18X, '   general pen. parameter =', ES11.4,        &
       &                 /, 18X, '     bound pen. parameter =', ES11.4,        &
       &                 /, 18X, '       maximum multiplier =', ES11.4,        &
       &                 /, 18X, ' ====================================== ')" )&
       &         inform%major_iter, pass, rho_g, rho_b, mult_max
...
!  Form and factorize the reference matrix
  ...
  10    CONTINUE
        ...
        CALL QPA_factorize_reference(                                          &
                    control, inform, dims, n, m, jumpto_factorize_reference,   &
                    ...
                    Abycol_val, S_val, DIAG, AINFO, FINFO, CNTLA, FACTORS )
...
!  Prepare for minor iteration
        ...
        IF ( printi ) WRITE( out,                                              &
           "( /, ' There ', A3, I8, ' degree', A1, ' of freedom' )" )          &
            TOOLS_is_are( dof ), dof, TOOLS_s( dof )
...
!  ------------------------------------------------------------------------ 
        ...
        minor : DO
          ...
          IF ( warmer_start ) THEN
            IF ( printi ) WRITE( out, "( /, 31X, 'Warm start' )" )
          ELSE
          ...
          dof = n - m_active
          IF ( printt ) WRITE( out, "( /, ' dof = ', I7 )" ) dof
          IF ( dof < 0 ) WRITE( out, "( ' dof = ', I7, ' iter = ', I6 )" )    &
            dof, iter
          IF ( iter == 0 ) THEN
            ...
            IF ( printi ) WRITE( out, 2060 ) inform%iter, inform%merit,        &
              inform%infeas_g + inform%infeas_b,                               &
              inform%num_g_infeas + inform%num_b_infeas, time
...
!  Find a point that satisfies the constraints
              ...
              CALL QPA_ir( K, FACTORS, K_part, S_perm( : K_part%k_ref ),       &
                           B( : K_part%k_ref ), RES( : K_part%k_ref ),         &
                           x_r0, y0, x_f0, z0, CNTLA, itref_max + 1,           &
                           out, printm, RES_print, sinfo )
              inform%factorization_status = sinfo
              IF ( printm ) WRITE( out, "( ' ' )" )
...
!  From this point, try to minimize the quadratic on the current working set
              ...
              IF ( .NOT. G_eq_H ) THEN
                ...
                CALL QPA_pcg( dims, n, S, RHS, R_pcg, X_pcg, P_pcg, S_perm,    &
                               ...
                               K, K_part, H_val, H_col, H_ptr, control, CNTLA, &
                               FACTORS, print_level - 1,                       &
...
!  modify the preconditioner and restart
                    ...
                    IF ( pcg_status == - 3 .AND. minor_start ) THEN
                      ...
                      IF ( printi ) WRITE( out, 2081 ) G_perturb, CNTLA%u
                      G_eq_H = .FALSE.
                      CNTLA%u = MIN( ten * CNTLA%u, half )
...
!  Compute the initial gradient
            ...
            IF ( printi ) THEN
              IF ( G_eq_H .OR. inform%iter > 1 ) THEN
                WRITE( out, 2060 ) inform%iter, inform%merit,                  &
                                   inform%infeas_g + inform%infeas_b,          &
                                   inform%num_g_infeas + inform%num_b_infeas,  &
                                   time
              ELSE
                WRITE( out, 2061 ) inform%iter, inform%merit,                  &
                                   inform%infeas_g + inform%infeas_b,          &
                                   inform%num_g_infeas + inform%num_b_infeas,  &
                                   pcg_iter, time
              END IF
...
!                 write(6,*) ' minor_start, pcount ', minor_start, pcount
                  IF ( printi ) WRITE( out, 2050 ) inform%iter,                &
                    inform%merit, t_opt, inform%infeas_g + inform%infeas_b,    &
                    inform%num_g_infeas + inform%num_b_infeas,                 &
...
! If the search direction is essentially rounding errors, reset it to zero
            ...
            IF ( printt ) WRITE( out, "( ' s vs y ', 2ES12.4 )" ) s_norm,      &
                             SQRT( SUM( S( n + 1 : n_all ) ** 2 ) )
...
!  Check the residual
            ...
            IF ( inform%factorization_status /= 0 ) THEN
              IF ( printt ) WRITE ( out, 2010 ) inform%factorization_status
              EXIT minor
            END IF
...
!  ================
            ...
            CALL QPA_add_constraint( QPA_add_constraint_status, control,       &
                     ...
                     SCU_info, SCU_data, K, CNTLA, FACTORS )
...
!  now consider constraints in the reference set
              ...
              IF ( j_min /= 0 ) THEN
                ...
                IF ( printt ) THEN
                  IF ( j <= m ) THEN
                    WRITE( out, 2120 ) 'y', j, mult_min
                  ELSE
                    WRITE( out, 2120 ) 'z', j - m, mult_min
                  END IF
                END IF
                GO TO 20
              END IF 
              ...
              DO i = SCU_mat%m, 1, - 1
                ...
                IF ( j > 0 ) THEN
                  ...
                  IF ( mult < mult_zero ) THEN
                    ...
                    IF ( printt ) THEN
                      IF ( j <= m ) THEN
                        WRITE( out, 2120 ) 'y', j, mult
                      ELSE
                        WRITE( out, 2120 ) 'z', j - m, mult
                      END IF
                    END IF
                    GO TO 20
                  END IF
...
!  Now set the multipliers for constraints in the working set 
            ...
            CALL CPU_TIME( time ) ; time = time - time_start 
            IF ( printt .OR. ( printi .AND. pcount == 0 ) ) WRITE( out, 2040 ) &
              precon
            IF ( printi ) WRITE( out, 2050 ) inform%iter, inform%merit, t_opt, &
              inform%infeas_g + inform%infeas_b,                               &
              inform%num_g_infeas + inform%num_b_infeas,                       &
              pcg_iter, sc_data, addel, time

            IF ( printi ) WRITE( out, "( /, 18X,                               &
       &             ' ======================================',                &
       &     /, 18X, '         Optimal Solution found',                        &
       &     /, 18X, '   general pen. parameter =', ES11.4,                    &
       &     /, 18X, '     bound pen. parameter =', ES11.4,                    &
       &     /, 18X, '       maximum multiplier =', ES11.4,                    &
       &     /, 18X, ' ======================================' )" )            &
              rho_g, rho_b, mult_max
            EXIT major
...
!  other constraints
          ...
          IF ( printi .AND. .NOT. (  minor_start .AND. pcount == 0 ) )         &
            WRITE( out, 2050 ) inform%iter, inform%merit, t_opt,               &
                inform%infeas_g + inform%infeas_b,                             &
                inform%num_g_infeas + inform%num_b_infeas,                     &
                pcg_iter, sc_data, addel, time
          pcount = MOD( pcount + 1, 50 )
...
!  Print active constraint residuals if required
       ...
       IF ( printi ) THEN
         WRITE( out, "( /, 18x, 'verifying optimality and feasibility ...' )" )
       END IF
...
!  Compute the resulting gradient
          ...
          IF ( printi .AND. .NOT. (  minor_start .AND. pcount == 0 ) ) THEN
            ...
            ELSE
              WRITE( out, 2061 ) inform%iter, inform%merit,                    &
                inform%infeas_g + inform%infeas_b,                             &
                inform%num_g_infeas + inform%num_b_infeas, pcg_iter, time
            END IF
          END IF
          ...
          CALL QPA_pcg( dims, n, S, RHS, R_pcg, X_pcg, P_pcg, S_perm,         &
                         RES, B, DX, VECTOR, PERM, SCU_mat, SCU_data,         &
                         K, K_part, H_val, H_col, H_ptr, control, CNTLA,      &
                         FACTORS, print_level - 1, cg_maxit, dof, .TRUE.,     &
                         inner_stop_absolute, inner_stop_relative, iii,       &
                         inform, pcg_iter, negative_curvature, pcg_status,    &
                         RES_print( : K_part%n_free + K_part%c_ref ) )
          itref_max = iii - 1
...
!  Compute the resulting gradient
        ...
        CALL CPU_TIME( time ) ; time = time - time_start 
        IF ( printi ) WRITE( out, 2040 ) precon
!       write(6,*) ' minor_start, pcount ', minor_start, pcount
        IF ( printi .AND. .NOT. (  minor_start .AND. pcount == 0 ) ) THEN
          IF ( G_eq_H ) THEN
            WRITE( out, 2060 ) inform%iter, inform%merit,                     &
...
!  Non-executable statements
 ...
 2040 FORMAT( /, '                                violation ',                 &
                 ' pr =', I2, ' Schur complt               CPU',               &
              /, '   iter   merit fn.   step    size  number',                 &
                 ' CG its dim +ve -ve   change      time', / )
 2050 FORMAT( I7, ES12.4, ES9.2, ES8.1, I6, I7, A12, A10, F9.2 )
 2060 FORMAT( I7, ES12.4, 9X, ES8.1, I6, 29X, F9.2 )
 2061 FORMAT( I7, ES12.4, 9X, ES8.1, I6, I7, 22X, F9.2 )
 2062 FORMAT( I7, 54X, A9 )
 2081 FORMAT( /, ' Preconditioner is inappropriate as it gave',                &
                 ' a negative inner-product ', //,                             &
                 ' Perturbing G', :, ' by ', ES12.4, ' and restarting',        &
                 ' (pivtol = ', ES9.2, ')', / )
 2120 FORMAT( '   ', A1, '(', I6, ') = ', ES12.4 )
...
!  Local variables
      ...
      INTEGER :: alloc_status, scu_status
...
!  Deallocate all allocated integer arrays
      ...
      IF ( ALLOCATED( data%IBREAK ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%IBREAK'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%C_up_or_low ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%C_up_or_low'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%X_up_or_low ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%X_up_or_low'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%PERM ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%PERM'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%Abycol_row ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%Abycol_row'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%Abycol_ptr ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%Abycol_ptr'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%S_row ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%S_row'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%S_col ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%S_col'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%S_colptr ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%S_colptr'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%K%row ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%K%row'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%K%col ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%K%col'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
...
!  Deallocate all allocated real arrays
      ...
      IF ( ALLOCATED( data%BREAKP ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%BREAKP'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%RES_l ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%RES_l'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%RES_u ) ) THEN
        DEALLOCATE( data%RES_u, STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%RES_u'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%A_norms ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%A_norms'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%PERT ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%PERT'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%GRAD ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%GRAD'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%VECTOR ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%VECTOR'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
     ...
     IF ( ALLOCATED( data%RHS ) ) THEN
      ...
      IF ( ALLOCATED( data%S_perm ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%S_perm'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%R_pcg ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%R_pcg'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%X_pcg ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%X_pcg'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%P_pcg ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%P_pcg'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%Abycol_val ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%Abycol_val'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%S_val ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%S_val'
          IF ( control%error > 0 )                                             &
            WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%K%val ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%K%val'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%RES_PRINT ) ) THEN
        DEALLOCATE( data%RES_PRINT, STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%RES_PRINT'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%DIAG ) ) THEN
        DEALLOCATE( data%DIAG, STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%DIAG'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%DX ) ) THEN
        DEALLOCATE( data%DX, STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%DX'
          IF ( control%error > 0 )                                             &
              WRITE( control%error, 2900 ) inform%bad_alloc, inform%alloc_status
        END IF
      END IF
...
!  Deallocate all arrays allocated within SCU
      ...
      IF ( ALLOCATED( data%SCU_mat%BD_col_start ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_col_start'
      ...
      IF ( ALLOCATED( data%SCU_mat%BD_val ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_val'
          IF ( control%error > 0 ) WRITE( control%error, 2900 )                &
               inform%bad_alloc, inform%alloc_status
        END IF
      END IF
      ...
      IF ( ALLOCATED( data%SCU_mat%BD_row ) ) THEN
        ...
        IF ( inform%alloc_status /= 0 ) THEN
          inform%status = GALAHAD_error_deallocate
          inform%bad_alloc = 'qpa: data%SCU_mat%BD_row'
          IF ( control%error > 0 ) WRITE( control%error, 2900 )                &
               inform%bad_alloc, inform%alloc_status
        END IF
      END IF
...
!  Deallocate all arrays allocated within SILS
      ...
      CALL SILS_finalize( data%FACTORS, data%CNTLA, alloc_status )
      IF ( alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate
        inform%bad_alloc = 'qpa: data%FACTORS'
        IF ( control%error > 0 )                                               &
             WRITE( control%error, 2900 ) 'data%FACTORS', alloc_status
      END IF
...
!-*-*-   Q P A _ R E M O V E _ D E P E N D E N T  S U B R O U T I N E   -*-*-
      ...
      SUBROUTINE QPA_remove_dependent( n, m, A_val, A_col, A_ptr, K,           &
                                       FACTORS, CNTLA, C_stat, B_stat,         &
                                       WORKING, control, inform, n_depen )
...
!  Local variables
      ...
      INTEGER :: i, ii, j, l, ll, nroots, out, n_free, m_working, pmax, pmin
      REAL :: time, dum
      REAL ( KIND = wp ) :: root1, root2, dmax, dmin, rmax, rmin, big
      REAL ( KIND = wp ) :: cntla_u, cntla_tolerance, scale
      LOGICAL ::  twobytwo, reallocate, scale_a
...
!  Automatic arrays
      ...
      out = control%out ; cntla_u = CNTLA%u ; cntla_tolerance = CNTLA%tolerance
      CNTLA%u = control%pivot_tol_for_dependencies 
      CNTLA%tolerance = epsmch ** 2
      CNTLA%pivoting = 1 ; CNTLA%lp = 0 ; CNTLA%wp = 0 ; CNTLA%mp = 0
...
!  of K. Also record the indices of the working constraints
      ...
      IF ( out > 0 .AND. control%print_level >= 1 ) THEN
        WRITE( out, 2000 ) m_working, m, n - n_free, n
        WRITE( out,                                                            &
         "( /, 7( ' -' ), ' test for rank defficiency ', 6( ' - ' ) )" )
      END IF
      ...
      IF ( n_free == 0 ) THEN
        IF ( m_working /= 0 ) THEN
          IF ( out > 0 .AND. control%print_level >= 1 )                        &
            WRITE( out, "( /, '  all general constraints are dependent ' )" )
          C_stat = 0
        END IF
        n_depen = m_working
        GO TO 800
...
!  Allocate the arrays for the analysis phase
      ...
      IF ( ALLOCATED( K%row ) ) THEN
        IF ( SIZE( K%row ) < K%ne ) THEN ; DEALLOCATE( K%row )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( K%row( K%ne ), STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: K%row' ; GO TO 900 ; END IF
      END IF

      reallocate = .TRUE.
      IF ( ALLOCATED( K%col ) ) THEN
        IF ( SIZE( K%col ) < K%ne ) THEN ; DEALLOCATE( K%col )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( K%col( K%ne ), STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: K%col' ; GO TO 900 ; END IF
      END IF

      reallocate = .TRUE.
      IF ( ALLOCATED( K%val ) ) THEN
        IF ( SIZE( K%val ) < K%ne ) THEN ; DEALLOCATE( K%val )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( K%val( K%ne ), STAT = inform%alloc_status )
        IF ( inform%alloc_status /= 0 ) THEN 
          inform%bad_alloc = 'qpa: K%val'; GO TO 900
        END IF
      END IF
...
!     CNTLA%MP = 6
!IF ( control%print_level >= 1 ) THEN
! CNTLA%ldiag = 10000 ; CNTLA%sp = 88
! CNTLA%lp = 88 ; CNTLA%wp = 88 ; CNTLA%mp = 88
! write(88,*) ' *************************** new call *********************'
!     WRITE( 89, * ) ' K%n, K%ne ', K%n, K%ne
!     WRITE( 89, * ) ' K%row ', K%row( : K%ne )
!     WRITE( 89, * ) ' K%col ', K%col( : K%ne )
!     WRITE( 89, * ) ' K%val ', K%val( : K%ne )
!END IF

      CALL SILS_analyse( K, FACTORS, CNTLA, AINFO )
...
!  Factorize the matrix
      ...
      CNTLA%liw = MAX( 2 * inform%factorization_integer, control%indmin )
      CNTLA%la = MAX( 2 * inform%factorization_real, control%valmin )

      CALL CPU_TIME( time ) 
      CALL SILS_factorize( K, FACTORS, CNTLA, FINFO )
...
!  Record warning conditions
      ...
      IF ( control%out > 0 .AND. control%print_level >= 2 )                    &
        WRITE( control%out, "( ' ** factorize time = ', F10.2 ) " ) dum
      inform%time%factorize = inform%time%factorize + dum

! Allocate temporary workspace

      ALLOCATE( D( 2, n_free + m_working ) )
...
!  The final 1x1 block
      ...
      IF ( out > 0 .AND. control%print_level >= 3 )                           &
          WRITE( out, "( ' largest diagonal = ', ES12.4 )" ) dmax
...
! Deallocate the temporary workspace
...
!  Reset the pivot tolerance and zero pivoting threshold to their initial values
  ...
  800 CONTINUE
      inform%status = GALAHAD_ok
      CNTLA%u = cntla_u ; CNTLA%tolerance = cntla_tolerance

      IF ( out > 0 .AND. control%print_level >= 1 ) THEN
        WRITE( out,                &
         "( /, 6( ' -' ), ' end of test for rank defficiency', 5( ' - ' ) )" )
        WRITE( out, 2000 ) m_working - n_depen, m, n - n_free, n
      END IF
...
! Deallocate any temporary workspace
...
!  Non-executable statements
 ...
 2000 FORMAT( /, I8, ' in working set from ', I8, ' constraints',              &
              /, I8, ' in working set from ', I8, ' simple bounds' )
...
!-*-  Q P A _ I T E R A T I V E _ R E F I N E M E N T  S U B R O U T I N E -*-
      ...
      SUBROUTINE QPA_iterative_refinement( K, SCU_mat, SCU_data, RHS, X,       &
                                           B, RES, DX, VECTOR, CNTLA,          &
                                           FACTORS, K_part, zeq0, itref_max,   &
...
!  Dummy arguments
      ...
      TYPE ( SILS_control ), INTENT( IN ) :: CNTLA
!     TYPE ( SILS_factors ), INTENT( INOUT ) :: FACTORS
...
!  Local variables
      ...
      INTEGER :: i, ii, j, l, jumpto, scu_status, info, iter
      REAL ( KIND = wp ) :: val
...
!  Solve the linear system
      ...
      DO
        ...
        CALL QPA_ir( K, FACTORS, K_part, VECTOR, B, RES, x_r0, y0, x_f0, z0,   &
                     CNTLA, itref_max - 1, out, print, RES_print, info )
        IF ( info /= 0 ) THEN
          inform%factorization_status = info
          EXIT
        END IF
        jumpto = jumpto + 1
      END DO
...
!-*-*-*-*-*-*-*-*-*-  Q P A _ _ i r  S U B R O U T I N E -*-*-*-*-*-*-*-*-*-
      ...
      SUBROUTINE QPA_ir( K, FACTORS, K_part, X, B, RES, x_r0, y0, x_f0, z0,    &
                         CNTLA, itref_max, out, print, RES_print, info )
...
!  Solve the linear system
      ...
      IF ( print ) WRITE( out, "( '  subnorm rhs      = ', ES12.4 )" )         &
          MAXVAL( ABS( B ) )
      CALL QPA_block_solve( K, FACTORS, K_part, X, x_r, y, x_f, z,             &
                            print, RES_print, out, CNTLA, info )
...
!  Find the correction
          ...
          END IF
          x_f = .FALSE. ; z = .TRUE.
          CALL QPA_block_solve( K, FACTORS, K_part, RES, x_r, y, x_f, z,      &
                                print, RES_print, out, CNTLA, info )
...
!-*-*-*-*-*-*-*-*-*-  Q P A _ B L O C K _ S O L V E   -*-*-*-*-*-*-*-*-*-*-
      ...
      SUBROUTINE QPA_block_solve( K, FACTORS, K_part, X, b_r, c, b_f, d,       &
                                  print, RES_print, out, CNTLA, info )
...
!   (  A_r           ) (  y  )   (  c  )
      ...
      IF ( .NOT. ( b_r .AND. c .AND. d ) ) THEN
        IF ( K%n > 0 ) THEN
          ...
          END IF
          CALL SILS_solve( K, FACTORS, X, CNTLA, SINFO )
          info = SINFO%flag
          IF ( print ) THEN
...
!-*-*-*-*-*-*-*-*-*-*-  Q P A _ P C G   S U B R O U T I N E  -*-*-*-*-*-*-*
      ...
      SUBROUTINE QPA_pcg( dims, n, SOL, RHS, R, X, P, R_perm, G_perm, B, DX,   &
                          ...
                          H_col, H_ptr, control, CNTLA, FACTORS, print_level,  &
                          cg_maxit, dof, start_from_x,                         &
...
!  Compute the stopping tolerance
        ...
        IF ( printi ) WRITE( control%out, "( /, ' stopping tolerence = ',      &
       &                    ES12.4 )" ) stop_tol
      END IF
...
!  Print details of the latest iteration
      ...
      IF ( printi ) THEN
        IF ( MOD( iter, 25 ) == 0 .OR. printm ) WRITE( control%out, 2000 )
        
        IF ( iter /= 0 ) THEN
          WRITE( control%out, "( I9, ES16.8, 3ES9.2, ES10.2 )" )               &
                 iter, f, pgnorm, alpha, norm_p, rayleigh
        ELSE
          WRITE( control%out, "( I9, ES16.8, ES9.2, 5X, '-',                   &
        &         8X, '-', 9X, '-' )" ) iter, f, pgnorm
        END IF
      END IF
...
!  Test for an interior approximate solution
      ...
      IF ( pgnorm <= stop_tol ) THEN
        IF ( printi ) WRITE( control%out,                                      &
             "( ' pgnorm ', ES12.4, ' < ', ES12.4 )" ) pgnorm, stop_tol
...
!  Compute the 2-norm of the search direction
      ...
      norm_p = TWO_NORM( n, P, 1 )

!  Test for convergence

      IF ( norm_p <= norm_p_tiny ) THEN
        IF ( printi ) WRITE( control%out,                                      &
             "( ' pnorm ', ES12.4, ' < ', ES12.4 )" ) norm_p, norm_p_tiny
        status = GALAHAD_ok ; EXIT
      END IF
...
!  Negative curvature encountered
      ...
      CASE( - 2 )
        ...
        IF ( printi )                                                          &
          WRITE( control%out, "( I9, 8X, '-', 12X, 2( '-', 8X ), '-', 3X,      &
         &                        ES10.2 )" ) iter, rayleigh
...
!       WRITE( control%out,  &
        ...
        "( /, ' The preconditioner appears to be indefinite.',                 &
     &        ' Inner product = ', ES12.4  )" ) rminvr

      CASE( - 4 )
        IF ( control%error > 0 .AND. print_level >= 1 ) WRITE( control%error,  &
           "( ' n = ', I6, ' is not positive ' )" ) n
      END SELECT
...
!  Non-executable statements
 ...
 2000 FORMAT( /, '     Iter        f         pgnorm    step    norm p     curv')
...
!-*-*-*-*-*-*-*-   Q P A _ F O R M _ S _ C   S U B R O U T I N E   -*-*-*-*-*-
      ...
      SUBROUTINE QPA_form_Schur_complement(                                    &
                          ...
                          ierr, factor, max_col, max_sc,                       &
                          control_error, control_print_level, control_valmin )
...
!  Dummy arguments
      ...
      INTEGER, INTENT( IN ) :: control_valmin, max_col, max_sc
...
!  Check for input errors
      ...
      IF ( n_free <= 0 ) THEN
        ierr = 1
        IF ( control_error > 0 .AND. control_print_level >= 1 )                &
          WRITE( control_error, 2010 ) n_free
        GO TO 900
      END IF
...
!  Allocate the arrays for the analysis phase
      ...
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( Abycol_val( A_ne ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
      ...
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( Abycol_row( A_ne ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
      ...
      IF ( ALLOCATED( Abycol_ptr ) ) THEN
        IF ( SIZE( Abycol_ptr ) /= n_free + 1 ) THEN 
          DEALLOCATE( Abycol_ptr )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( Abycol_ptr( n_free + 1 ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
...
!  Compute the length of the largest column as well as the average length
        ...
        max_len = MAXVAL( Abycol_ptr( 2 : ) - Abycol_ptr( : n_free ) )
        IF ( control_error > 0 .AND. control_print_level >= 1 )                &
           WRITE( control_error, 2000 ) max_len,                               &
          float( ( Abycol_ptr( n_free + 1 ) - 1 ) ) / float( n_free ),         &
          max_col, COUNT( Abycol_ptr( 2 : ) - Abycol_ptr( : n_free )           &
           > max_col )
      ELSE
        max_len = 0
      END IF
...
!  Check that the largest column is not too long
      ...
      IF ( factor == 0 .AND. max_len > max_col .AND. m > max_sc ) THEN
        IF ( control_error > 0 .AND. control_print_level >= 1 )                &
          WRITE( control_error, 2050 ) max_col
        ierr = 5
        factor = 2
...
!  Continue allocating the arrays for the analysis phase
      ...
      ls = max( A_ne + m, 2 * A_ne, control_valmin )

      reallocate = .TRUE.
      IF ( ALLOCATED( S_row ) ) THEN
        IF ( SIZE( S_row ) < ls ) THEN ; DEALLOCATE( S_row )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( S_row( ls ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN
          ierr = 5 ; GO TO 900
        END IF
      END IF
      ...
      reallocate = .TRUE.
      IF ( ALLOCATED( S_col ) ) THEN
        IF ( SIZE( S_col ) < ls ) THEN ; DEALLOCATE( S_col )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( S_col( ls ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
      ...
      reallocate = .TRUE.
      IF ( ALLOCATED( S_val ) ) THEN
        IF ( SIZE( S_val ) < ls ) THEN ; DEALLOCATE( S_val )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( S_val( ls ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
      ...
      reallocate = .TRUE.
      IF ( ALLOCATED( S_colptr ) ) THEN
        IF ( SIZE( S_colptr ) /= m + 1 ) THEN ; DEALLOCATE( S_colptr )
        ELSE ; reallocate = .FALSE.
        END IF
      END IF
      IF ( reallocate ) THEN 
        ALLOCATE( S_colptr( m + 1 ), STAT = alloc_status )
        IF ( alloc_status /= 0 ) THEN 
          ierr = 5 ; GO TO 900
        END IF
      END IF
...
!  Non-executable statements
 ...
 2010 FORMAT( ' ** Error return from QPA_form_Schur_complement', /,            &
              '    Value of n (number of rows of A) is set to ', I10, /,       &
              '    but n must be at least 1 ' )
 2020 FORMAT( ' ** Error return from QPA_form_Schur_complement', /,            &
              '    Value of m (number of columns of A) is set to ', I10, /,    &
              '    but m must be at least 1 ' )
 2050 FORMAT( ' ** The maximum column length in A is larger than', /,          &
...
!-*-   Q P A _ F A C T O R I Z E _ R E F E R E N C E   S U B R O U T I N E   -*-
      ...
      SUBROUTINE QPA_factorize_reference(                                       &
                    control, inform, dims, n, m, jumpto, k_n_max, print_level,  &
...
!  Local variables
      ...
      REAL :: time, dum
      LOGICAL :: reallocate,  modify_k, twobytwo
...
!  Print a header indicating the method selected
  ...
  10  CONTINUE
      ...
      IF ( printi ) THEN
        SELECT CASE( precon )
          CASE( : - 1, 1 ) ; WRITE( control%out, 2410 )
          CASE( 0 ) ; WRITE( control%out, 2400 )
          CASE( 2 ) ; WRITE( control%out, 2420 )
          CASE( 3 ) ; WRITE( control%out, 2430 ) nsemib
          CASE( 4 ) ; WRITE( control%out, 2440 ) 
          CASE( 5 ) ; WRITE( control%out, 2450 ) nsemib
        END SELECT
  
        IF ( factor == 0 .OR. factor == 1 ) THEN
          WRITE( control%out, "( '  Schur-complement method used ',          &
         &       '(pivot tol =', ES9.2, ')' )" ) CNTLA%u
        ELSE
...
!  Compute the total space required for the reference matrix
      ...
      IF ( printt )                                                          &
        WRITE( out, "( /, ' n, n_free, n_fixed            = ', 3I8,          &
     &                 /, ' k_free_od, k_free_d, k_free_p = ', 3I8,          &
     &                 /, ' k_fixed_od, k_fixed_d         = ', 2I8,          &
     &                 /, ' c_ref, m_ref                  = ', 2I8  )" )     &
        n, K_part%n_free, K_part%n_fixed, K_part%k_free_od,                  &
        K_part%k_free_d, K_part%k_free_p, K_part%k_fixed_od,                 &
        K_part%k_fixed_d, K_part%c_ref, K_part%m_ref
            ...
            IF ( B_stat( i ) == 0 ) THEN  ! ** free variable
              DO l = H_ptr( i ), H_ptr( i + 1 ) - 2
                ...
                IF ( B_stat( j ) == 0 ) THEN ! ** free variable
                  ...
                  IF ( factor == 0 .OR. factor == 1 ) THEN
                    IF ( printi ) WRITE( control%out, 2390 )
                    factor = 2
                  END IF
                ELSE ! ** fixed variable
                  K_part%k_fixed_od = K_part%k_fixed_od + 1
              ...
              IF ( ( factor == 0 .OR. factor == 1 ) .AND.                 &
                     H_val( l ) == zero ) THEN
                IF ( printi ) WRITE( control%out, 2380 )
                factor = 2
              END IF
            ELSE  ! ** fixed variable
              DO l = H_ptr( i ), H_ptr( i + 1 ) - 2
...
!  reference matrix
      ...
      IF ( printi .AND. K_part%k_free_od + K_part%n_free /= K_part%k_free_d) &
        THEN ; WRITE( out, "( ' too few diagonals ', 2I6 )" )                &
      ...
      IF ( factor == 0 .OR. factor == 1 ) THEN
        IF ( K_part%n_free > 0 .AND. K_part%c_ref >= 0 ) THEN
          CALL QPA_form_Schur_complement(                                   &
                          ...
                          Abycol_ne, Abycol_val, Abycol_row, Abycol_ptr,     &
                          S_ne, S_val, S_row, S_col, S_colptr,               &
                          ierr, factor, max_col, control%max_sc,             &
                          control%error, print_level, control%valmin )
          IF ( ierr == 5 ) THEN
            IF ( printi ) WRITE( control%out, 2381 )
            factor = 2
          END IF
        END IF
      END IF
...
!  Analyse the sparsity pattern of the free sub-block of the reference matrix
  ...
  20  CONTINUE
      IF ( K%n > 0 ) THEN
        ...
        CALL CPU_TIME( dum ) ; dum = dum - time
        IF ( control%out > 0 .AND. control%print_level >= 2 )                  &
          WRITE( control%out, "( ' ** analysis time = ', F10.2 ) " ) dum
        inform%time%analyse = inform%time%analyse + dum
...
!  Check for error returns
        ...
        inform%factorization_status = AINFO%flag
        IF ( AINFO%flag < 0 ) THEN
          IF ( printe ) WRITE( control%error, 2090 ) AINFO%flag,             &
                                                     'SILS_analyse'
          IF ( AINFO%flag == - 3 ) THEN
            IF ( auto_prec ) THEN
              IF ( prec_hist == 1 ) THEN
                precon = 2
                GO TO 10
        ...
        ELSE IF ( AINFO%flag > 0 ) THEN 
          IF ( printt ) WRITE( control%out, 2100 )                           &
            AINFO%flag, 'SILS_analyse'
        END IF

        IF ( printi ) WRITE( control%out,                                    &
        "( /, ' nnz, real/integer space anticipated for factors ', 3I10 )" ) &
             K%ne, AINFO%nrladu, AINFO%niradu
...
!  Test that the factorization succeeded
        ...
        zeig = 0
        IF ( FINFO%flag < 0 ) THEN
          IF ( printe ) WRITE( control%error, 2090 ) FINFO%flag,             &
                                                     'SILS_factorize'
...
!  Record warning conditions
        ...
        END IF 
        IF ( printi ) WRITE( control%out,                                    &
          "( ' nnz, real/integer space required for factors    ', 3I10 )" )  &
             K%ne, FINFO%nrlbdu, FINFO%nirbdu
...
!  Test that the problem is convex in the null-space of the constraints
        ...
        modify_k = zeig + FINFO%neig > K_part%c_ref
...
!  Determine the block diagonal part of the factors
          ...
          CALL SILS_enquire( FACTORS, D = DIAG )
...
!  The final 1x1 block
            ...
            END IF
          END DO
          IF ( printt .AND. modify_k ) WRITE( control%out, 2071 )
        ...
        IF ( modify_k ) THEN 
          ...
          K%val( K_part%k_free_od + 1 : K_part%k_free_p )                      &
            = K%val( K_part%k_free_od + 1 : K_part%k_free_p ) + hmax
          IF ( printi ) WRITE( out, 2080 ) FINFO%neig, zeig, K_part%c_ref,     &
            TOOLS_s( K_part%c_ref ), G_perturb, CNTLA%u
...
!  Allocation error
  ...
  900 CONTINUE 
      inform%status = - flags - ABS( inform%alloc_status ) ; jumpto = 1
      IF ( printi ) WRITE( control%out, 2900 )                                 &
        inform%bad_alloc, inform%alloc_status
      IF ( control%out > 0 .AND. control%print_level >= 5 )                    &
        WRITE( control%out, "( ' leaving QPA_solve_main ' )" )
...
!  Non-executable statements
 ...
 2071 FORMAT( ' ** Matrix has at least one zero eigenvalue ' )
 2080 FORMAT( /, ' Preconditioner is inappropriate as it has ',                &
              /, I6, ' negative and ', I6, ' zero eigenvalues as opposed to',  &
              /, I6, ' negative one', A1,                                      &
              //,    ' Perturbing G', :, ' by', ES14.6, ' and restarting',     &
                     ' (pivtol = ', ES9.2, ')', / )
 2090 FORMAT( '   **  Error return ', I6, ' from ', A15 ) 
 2100 FORMAT( '   **  Warning ', I6, ' from ', A15 ) 
 2380 FORMAT( /, ' zero diagonal ... switching to augmented system method ', / )
 2381 FORMAT( /, ' Schur complement failure ... switching to augmented',       &
                 ' system method ', / )
 2390 FORMAT( /,' off-diagonal term ... switching to augmented system method' )
 2400 FORMAT( /, '  Automatic preconditioner ' )
 2410 FORMAT( /, '  Full Hessian ' )
 2420 FORMAT( /, '  Identity Hessian ' )
 2430 FORMAT( /, '  Band (semi-bandwidth ', I3, ') Hessian ' )
 2440 FORMAT( /, '  Identity Hessian for free variables, full for remainder ' )
 2450 FORMAT( /, '  Band (semi-bandwidth ', I3, ') Hessian for free',          &
                 '  variables, full for remainder ' )
 2900 FORMAT( ' ** Message from -QPA_factorize_reference-', /,                 &
                 ' Allocation error, for ', A, ', status = ', I6 ) 
...
!-*-*-*-*-   Q P A _ N E W _ R E F E R E N C E   S U B R O U T I N E   -*-*-*-*-
      ...
      SUBROUTINE QPA_new_reference_set( control, inform, n, m, K_part,         &
                                        SCU_mat, out, m_link, pcount, printd,  &
                                        warmer_start, check_dependent, C_stat, &
...
!  A general constraint occurs after a bound; swap them
      ...
      IF ( warmer_start .OR. check_dependent ) THEN
        ii = 0
        DO 
          CALL QPA_remove_dependent( n, m, A_val, A_col, A_ptr, K,             &
                                     FACTORS, CNTLA, C_stat, B_stat, WORKING,  &
                                     control, inform, n_depen )
          IF ( n_depen == 0 ) THEN
            IF ( ii > 0 ) THEN
...
!  Local variables
      ...
      INTEGER :: i, ii, j, jj, ll, lll, nbd, sinfo
...
!  Update the Schur complement
        ...
        DO
          ...
          CALL QPA_ir( K, FACTORS, K_part, VECTOR, B, RES, x_r0, y0, x_f0, z0, &
                       CNTLA, itref_max, out, printm, RES_print, sinfo )
          x_r0 = .FALSE. ; y0 = .FALSE. ; x_f0 = .FALSE. ; z0 = .FALSE.
...
!  Local variables
      ...
      INTEGER :: i, ii, j, jj, jjj, ll, nbd, sinfo
      LOGICAL :: x_r0, y0, x_f0, z0
      CHARACTER ( LEN = 2 ) :: uplow
