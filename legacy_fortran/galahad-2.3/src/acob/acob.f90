! THIS VERSION: GALAHAD 2.2 - 28/05/2008 AT 14:30 GMT.

!-*-*-*-*-*-*-*-*-  G A L A H A D _ A C O B   M O D U L E  *-*-*-*-*-*-*-*-*-*-
...
!   http://galahad.rl.ac.uk/galahad-www/specs.html
   ...
   MODULE GALAHAD_ACOB_double

!     -------------------------------------------------
!    |                                                 |
!    | ACOB, an adaptive cubic-overestimation          |
...
!NOT95USE GALAHAD_CPU_time
     ...
     PRIVATE
     PUBLIC :: ACOB_initialize, ACOB_read_specfile, ACOB_solve,              &
               ACOB_terminate, NLPT_problem_type, NLPT_userdata_type,          &
               ACOB_projection
...
!--------------------------
     ...
     END TYPE ACOB_data_type

     TYPE, PUBLIC :: ACOB_control_type
     ...
     END TYPE ACOB_control_type

     TYPE, PUBLIC :: ACOB_time_type
       REAL :: total, preprocess, analyse, factorize, solve
     END TYPE

     TYPE, PUBLIC :: ACOB_inform_type
       ...
       TYPE ( ACOB_time_type ) :: time
       TYPE ( SBLS_inform_type ) :: SBLS_inform
       TYPE ( GLRT_inform_type ) :: GLRT_inform
     END TYPE ACOB_inform_type
...
!-*-  G A L A H A D -  A C O B _ I N I T I A L I Z E  S U B R O U T I N E  -*-

     SUBROUTINE ACOB_initialize( data, control )
...
!-----------------------------------------------
     ...
     TYPE ( ACOB_data_type ), INTENT( OUT ) :: data
     TYPE ( ACOB_control_type ), INTENT( OUT ) :: control
...
!  End of subroutine ACOB_initialize

     END SUBROUTINE ACOB_initialize

!-*-*-*-*-   A C O B _ R E A D _ S P E C F I L E  S U B R O U T I N E  -*-*-*-

     SUBROUTINE ACOB_read_specfile( control, device, alt_specname )

!  Reads the content of a specification file, and performs the assignment of 
!  values associated with given keywords to the corresponding control parameters

!  The default values as given by ACOB_initialize could (roughly) 
!  have been set as:

! BEGIN ACOB SPECIFICATIONS (DEFAULT)
...
!-----------------------------------------------
     ...
     TYPE ( ACOB_control_type ), INTENT( INOUT ) :: control        
...
!-----------------------------------------------
     ...
     CHARACTER( LEN = 16 ), PARAMETER :: specname = 'ACO          '
...
!  Real key-words
     ...
     spec( 23 )%keyword = 'successful-iteration-tolerence'
     spec( 24 )%keyword = 'very-successful-iteration-tolerence'
...
!  Read the controls for the preconditioner and iterative solver
     ...
     END SUBROUTINE ACOB_read_specfile

!-*-*-*-  G A L A H A D -  A C O B _ s o l v e  S U B R O U T I N E  -*-*-*-

     SUBROUTINE ACOB_solve( nlp, control, inform, data,                        &
                             eval_F, eval_G, eval_H, userdata )

!  *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*

!  ACOB_solve, a method for finding a local unconstrained minimizer of a 
...
!-----------------------------------------------
     ...
     TYPE ( ACOB_control_type ), INTENT( INOUT ) :: control
     TYPE ( ACOB_inform_type ), INTENT( INOUT ) :: inform
     TYPE ( ACOB_data_type ), INTENT( INOUT ) :: data
     TYPE ( NLPT_userdata_type ), OPTIONAL, INTENT( INOUT ) :: userdata
...
!----------------------------------
     ...
     INTERFACE
       SUBROUTINE eval_F( status, f, X, userdata )
     ...
     INTERFACE
       SUBROUTINE eval_H( status, Hval, X, userdata )
       USE GALAHAD_NLPT_double, ONLY: NLPT_userdata_type
       INTEGER, PARAMETER :: wp = KIND( 1.0D+0 )
       INTEGER, INTENT( OUT ) :: status
       REAL ( KIND = wp ), DIMENSION( : ), INTENT( IN ) :: X
...
!  Ensure that the initial point is feasible
     ...
     CALL ACOB_projection( n, nlp%X, nlp%X_l, nlp%X_u, data%X_plus )
     nlp%X( : n ) = data%X_plus( : n )

! evaluate the objective function at the initial point

     CALL eval_F( eval_stat, inform%obj, nlp%X( : n ), userdata )
     inform%f_eval = inform%f_eval + 1

!  evaluate the gradient of the objective function

     CALL eval_G( eval_stat, nlp%G( : n ), nlp%X( : n ), userdata )
     inform%g_eval = inform%g_eval + 1

!  compute the norm of the projected gradient

     CALL ACOB_projection( n, nlp%X - nlp%G, nlp%X_l, nlp%X_u, data%X_plus )
     inform%norm_pg = MAXVAL( ABS( nlp%X - data%X_plus ) )
 
!  evaluate the initial Hessian matrix
    
     CALL eval_H( eval_stat, nlp%H%val( : nlp%H%ne ), nlp%X( : n ), userdata )
...
!  Step 1.1: compute a point x(t) on the projected gradient path
         ...
         CALL ACOB_projection( n, nlp%X - t * nlp%G, nlp%X_l, nlp%X_u,        &
                                data%X_plus )
...
!  Evaluate the model at x(t)
         ...
         CALL ACOB_model( n, data%S( : n ), inform%obj, nlp%G( : n ),  nlp%H, &
...
!  compute the projection of x + s onto the feasible region
           ...
           CALL ACOB_projection( n, nlp%X + data%S, nlp%X_l, nlp%X_u,         &
                                  data%X_plus )
...
!  evaluate the model at the correction
           ...
           CALL ACOB_model( n, data%S( : n ), inform%obj, nlp%G( : n ),       &
...
!  evaluate the objective function at the trial point
       ...
       CALL eval_F( eval_stat, f_plus, data%X_plus( : n ), userdata )
       inform%f_eval = inform%f_eval + 1

!  compute the change in the model

       CALL ACOB_model( n, data%S( : n ), zero, nlp%G( : n ),  nlp%H,        &
...
!  evaluate the Hessian matrix
         ...
         CALL eval_H( eval_stat, nlp%H%val( : nlp%H%ne ), nlp%X( : n ),        &
                      userdata )
...
!  Print the solution
    ...
    &  ' Gradient evals =     ',bn, I12, '       Time            = ', F12.2 )") &
     ...
     IF ( inform%status /= 0 ) THEN
       WRITE( control%error, "( ' ** Message from -ACOB_solve- ',              &
...
!  End of subroutine ACOB_solve

     END SUBROUTINE ACOB_solve

!-*-*-*-*-  G A L A H A D -  A C O B _ m o d e l   S U B R O U T I N E -*-*-*-

     SUBROUTINE ACOB_model( n, X, f, C, H, sigma, l_model, q_model, c_model, W )
...
!  End of subroutine ACOB_model

     END SUBROUTINE ACOB_model

!-*-*-  G A L A H A D -  A C O B _ t e r m i n a t e  S U B R O U T I N E -*-*-

     SUBROUTINE ACOB_terminate( data, control, inform )
...
!-----------------------------------------------
     ...
     TYPE ( ACOB_data_type ), INTENT( INOUT ) :: data
     TYPE ( ACOB_control_type ), INTENT( IN ) :: control
     TYPE ( ACOB_inform_type ), INTENT( INOUT ) :: inform
...
!  End of subroutine ACOB_terminate

     END SUBROUTINE ACOB_terminate

!-*-  G A L A H A D -  A C O B _ p r o j e c t i o n   S U B R O U T I N E -*-

     SUBROUTINE ACOB_projection( n, C, X_l, X_u, X, iter, A, b )
...
!   &   ' iterations' )" ) pr_feas, du_feas, iter
     ...
     END SUBROUTINE ACOB_projection

!  End of module GALAHAD_ACOB

   END MODULE GALAHAD_ACOB_double
