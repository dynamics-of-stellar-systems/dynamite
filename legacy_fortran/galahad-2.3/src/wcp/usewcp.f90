...
!    ------------------------------------------------------------
!    | CUTEr/AMPL interface to WCP, an interior-point algorithm |
...
!         x_l <=  x <= x_u
!
!  specified by CUTEr using the GALAHAD package GALAHAD_WCP
!
!  --------------------------------------------------------
...
!  Allocate suitable arrays
      ...
      ALLOCATE( prob%C_l( mmax ), prob%C_u( mmax ), prob%Y_l( mmax ),          &
                prob%Y_u( mmax ), CNAME( mmax ), EQUATN( mmax ),               &
                LINEAR( mmax ), STAT = alloc_stat )
      IF ( alloc_stat /= 0 ) THEN
...
!  separable function.
      ...
      CALL CSETUP( input, out, n, m, prob%X, prob%X_l, prob%X_u, nmax, EQUATN, &
                   LINEAR, prob%Y_l, prob%C_l, prob%C_u, mmax, .FALSE.,        &
                   .FALSE., .FALSE. )
      DEALLOCATE( LINEAR )
...
!  Evaluate the linear terms of the constraint functions
      ...
      CALL CSGR( n, m, .FALSE., mmax, prob%Y_l, prob%X0, nea, la, prob%A%val,  &
...
!  Allocate and initialize dual variables.
      ...
      ALLOCATE( prob%Z_l( nmax ), prob%Z_u( nmax ), STAT = alloc_stat )
      IF ( alloc_stat /= 0 ) THEN
...
!  Set all default values, and override defaults if requested
      ...
      CALL WCP_initialize( data, WCP_control )
      IF ( is_specfile )                                                       &
        CALL WCP_read_specfile( WCP_control, input_specfile )

      WCP_control%restore_problem = 2

      control%print_level = WCP_control%print_level
      control%out         = WCP_control%out
      control%out_error   = WCP_control%error
...
!  Apply the scaling factors
        ...
        IF ( scale >= - 5 ) THEN
          CALL SCALING_apply_factors( prob%n, prob%m, prob%H%val, prob%H%col,  &
                                      ...
                                      prob%X_u, prob%C_l, prob%C_u, prob%Y_l,  &
                                      prob%Y_u, prob%Z_l, prob%Z_u, infinity,  &
                                      SH, SA, .TRUE. )
        ELSE
        ...
        IF ( scale > 0 ) THEN
          IF ( scale <= 5 ) THEN
            CALL SCALING_apply_factors( prob%n, prob%m, prob%H%val, prob%H%col,&
                                        prob%H%ptr, prob%A%val, prob%A%col,    &
                                        prob%A%ptr, prob%G, prob%X, prob%X_l,  &
                                        prob%X_u,  prob%C_l, prob%C_u,         &
                                        prob%Y_l, prob%Y_u, prob%Z_l,          &
                                        prob%Z_u, infinity, SH, SA, .FALSE.,   &
                                        C = prob%C )
            DEALLOCATE( SH, SA )
          ELSE
...
!  Compare the variants used so far
      ...
      IF ( write_result_summary ) THEN
        ...
        WRITE( rfiledevice, 2190 )                                             &
           ...
           WCP_inform%c_implicit, WCP_inform%y_implicit, iter, status, timet
      END IF
