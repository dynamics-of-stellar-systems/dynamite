! THIS VERSION: GALAHAD 2.2 - 27/05/2008 AT 16:30 GMT.
...
!--------------------------
      ...
      TYPE, PUBLIC :: LSTR_control_type
        INTEGER :: error, out, print_level, itmax, itmax_on_boundary
        INTEGER :: extra_vectors
        REAL ( KIND = wp ) :: stop_relative, stop_absolute, fraction_opt
        LOGICAL :: steihaug_toint, space_critical, deallocate_error_fatal
      ...
      TYPE, PUBLIC :: LSTR_inform_type
        ...
        REAL ( KIND = wp ) :: multiplier, x_norm, r_norm, Atr_norm, biter_mean
      ...
      TYPE, PUBLIC :: LSTR_data_type
        ...
        LOGICAL :: printi, printd, interior, header, try_warm
        LOGICAL :: prev_steihaug_toint, save_vectors
...
!-*-*-*-*-*-  L S T R _ I N I T I A L I Z E   S U B R O U T I N E   -*-*-*-*-*-
      ...
      SUBROUTINE LSTR_initialize( data, control )
...
!            itmax_on_boundary   maximum number of iterations beyond the
!                              Steihaug-Toint point allowed
...
!-----------------------------------------------
      ...
      TYPE ( LSTR_DATA_TYPE ), INTENT( OUT ) :: data
...
!  Local variables
      ...
      CHARACTER( LEN = 16 ), PARAMETER :: specname = 'LSTR            '
...
!  Integer key-words
      ...
      spec(  5 )%keyword = 'maximum-number-of-boundary-iterations'
...
!  Set integer values
      ...
      CALL SPECFILE_assign_value( spec( 4 ), control%itmax,                     &
                                  control%error )
      CALL SPECFILE_assign_value( spec( 5 ), control%itmax_on_boundary,         &
                                  control%error )
...
!                        initial entry or 5 on a re-entry when only radius has
...
!  Allocate workspace for the sequence of smallest function values & multipliers
...
!  Check for termination
        ...
        IF ( inform%ATr_norm < data%stop ) THEN
