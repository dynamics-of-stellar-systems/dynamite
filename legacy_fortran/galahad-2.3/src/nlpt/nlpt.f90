...
!  ===================================
      ...
      TYPE, PUBLIC :: NLPT_userdata_type
         ...
         COMPLEX, ALLOCATABLE, DIMENSION( : ) :: complex
         ...
         COMPLEX, POINTER, DIMENSION( : ) :: complex_pointer => null( )
...
!  Local variables
   ...
   IF ( print_level >= ACTION ) THEN         
      ...
      END IF
   ELSE
      WRITE( out, 102 ) TWO_NORM( problem%n, problem%x, 1 )
      WRITE( out, 101 ) problem%f, TWO_NORM( problem%n, problem%g, 1 )
      max_violation = ZERO
      IF ( ALLOCATED( problem%x_l ) .AND. ALLOCATED( problem%x_u ) ) THEN
         DO j = 1, problem%n
