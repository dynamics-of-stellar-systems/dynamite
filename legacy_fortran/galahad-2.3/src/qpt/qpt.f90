...
!-----------------------------------------------
      ...
      TYPE, PUBLIC :: QPT_problem_type
        ...
        INTEGER :: Hessian_kind             ! whether H or WEIGHT need be given
        INTEGER :: gradient_kind            ! kind of gradient
        ...
        INTEGER :: c_u_start
        INTEGER :: c_u_end
        REAL ( KIND = wp ) :: f              ! constant term
        REAL ( KIND = wp ) :: q              ! value of the objective
        REAL ( KIND = wp ) :: theta_max      ! upper bound on parametric range
        REAL ( KIND = wp ) :: theta          ! current value of parameter
        REAL ( KIND = wp ) :: rho_g          ! penalty parameter for general
                                             ! linear constraints for l_1QPs
        REAL ( KIND = wp ) :: rho_b          ! penalty parameter for simple
