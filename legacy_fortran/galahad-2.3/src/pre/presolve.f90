...
!                            10.**(-4) in single precision.
         ...
         REAL ( KIND = wp ) :: z_accuracy       ! INTENT( IN )
...
!                            10.**(-4) in single precision.
         ...
         REAL ( KIND = wp ) :: infinity         ! INTENT( IN )
...
!                   Default: SILENT
         ...
         LOGICAL :: dual_transformations        ! INTENT( IN )
...
!                   Default: .TRUE.
         ...
         LOGICAL :: redundant_xc                ! INTENT( IN )
...
!                   Default: .TRUE.
         ...
         INTEGER :: primal_constraints_freq     ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: dual_constraints_freq       ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: singleton_columns_freq      ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: doubleton_columns_freq      ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: unc_variables_freq          ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: dependent_variables_freq    ! INTENT( IN )
...
!                   Default: 1
         ...
         INTEGER :: sparsify_rows_freq          ! INTENT( IN )
...
!                   Default: KEEP
         ...
         CHARACTER( LEN = 30 ) :: transf_file_name   ! INTENT( IN )
...
!                   Default: LEAVE_AS_IS
         ...
         INTEGER :: final_x_bounds              ! INTENT( IN )
...
!                         PRESOLVE (except INITIALIZE).
         ...
         INTEGER :: final_z_bounds              ! INTENT( IN )
...
!                         PRESOLVE (except INITIALIZE).
         ...
         INTEGER :: final_c_bounds              ! INTENT( IN )
...
!                      to that of control%final_x_bounds.
         ...
         INTEGER :: final_y_bounds              ! INTENT( IN )
...
!                         PRESOLVE (except INITIALIZE).
         ...
         INTEGER :: check_primal_feasibility    ! INTENT( IN )
...
!                   Default: NONE
         ...
         INTEGER :: check_dual_feasibility      ! INTENT( IN )
...
!                   Default: .TRUE.
         ...
         LOGICAL :: get_x_bounds                ! INTENT( IN )
...
!                   Default: .TRUE. 
         ...
         LOGICAL :: get_z_bounds                ! INTENT( IN )
...
!                   Default: .TRUE.
         ...
         LOGICAL :: get_c_bounds                ! INTENT( IN )
...
!                   Default: .TRUE.
         ...
         LOGICAL :: get_y_bounds                ! INTENT( IN )
...
!                         PRESOLVE (except INITIALIZE)
         ...
         REAL ( KIND = wp ) :: pivot_tol        ! INTENT( IN )
...
!                            10.**(-6)  in single precision.
         ...
         REAL ( KIND = wp ) :: min_rel_improve  ! INTENT( IN )
...
!                            10.**(-6)  in single precision.
         ...
         REAL ( KIND = wp ) :: max_growth_factor ! INTENT( IN )
...
!                       incompatible value of prob%H_ne
         ...
         INTEGER :: nbr_transforms                   ! INTENT( OUT )
...
!----------------------
      ...
      REAL ( KIND = wp ), PRIVATE, PARAMETER :: TEN_sp  = 10.0_sp
      REAL ( KIND = wp ), PRIVATE, PARAMETER :: TEN_dp  = 10.0_dp
...
!    The value of INFINITY
      ...
      IF ( wp == sp ) THEN
         control%infinity = DEF_INFINITY_sp
      ELSE
         control%infinity = DEF_INFINITY_dp
      END IF
...
!     Maximum number of analysis passes
      ...
      control%max_nbr_passes = DEF_MAX_NBR_PASSES
...
!     Maximum number of problem transformations
      ...
      control%max_nbr_transforms = DEF_MAX_NBR_TRANSF
...
!     Size of the transformation buffer in memory
      ...
      control%transf_buffer_size = DEF_MAX_T_BUFFER
...
!     on disk
      ...
      control%transf_file_name = DEF_transf_file_name
...
!     Unit number for writing/reading the transformation file
      ...
      control%transf_file_nbr = DEF_TRANSF_FILE_NBR
...
!     KEEP or DELETE status for the transformation files
      ...
      control%transf_file_status = KEEP
...
!     Tolerance for pivoting in A
      ...
      IF ( wp == sp ) THEN
         control%pivot_tol = DEF_PIVOT_TOL_sp
      ELSE
         control%pivot_tol = DEF_PIVOT_TOL_dp
      END IF
...
!     Minimal relative bound improvement
      ...
      IF ( wp == sp ) THEN
         control%min_rel_improve = DEF_MRBI_sp
      ELSE
         control%min_rel_improve = DEF_MRBI_dp
      END IF
...
!     Maximum growth factor between original and reduce problems
      ...
      IF ( wp == sp ) THEN
         control%max_growth_factor = DEF_MAX_GROWTH_sp
      ELSE
         control%max_growth_factor = DEF_MAX_GROWTH_dp
      END IF
...
!     Relative accuracy on the linear constraints
      ...
      IF ( wp == sp ) THEN
         control%c_accuracy = DEF_C_ACC_sp
      ELSE
         control%c_accuracy = DEF_C_ACC_dp
      END IF
...
!     Relative accuracy on the dual variables
      ...
      IF ( wp == sp ) THEN
         control%z_accuracy = DEF_Z_ACC_sp
      ELSE
         control%z_accuracy = DEF_Z_ACC_dp
      END IF
...
!     Level of primal feasibility check at RESTORE
      ...
      control%check_primal_feasibility      = NONE
...
!     1) analysis of the primal constraints
      ...
      control%primal_constraints_freq = DEF_AN_PRIMAL_FREQ
...
!     2) analysis of the dual constraints
      ...
      control%dual_constraints_freq = DEF_AN_DUAL_FREQ
...
!     3) analysis of the singleton columns
      ...
      control%singleton_columns_freq = DEF_AN_SING_FREQ
...
!     4) analysis of the doubleton columns
      ...
      control%doubleton_columns_freq = DEF_AN_DOUB_FREQ
...
!     5) analysis of the linearly unconstrained variables
      ...
      control%unc_variables_freq = DEF_UNC_VARS_FREQ
...
!     6) analysis of the linearly dependent variables
      ...
      control%dependent_variables_freq = DEF_DEP_COLS_FREQ
...
!     7) row sparsification frequency
      ...
      control%sparsify_rows_freq = DEF_SPARSIFY_FREQ
...
!     Strategy for presolve termination
      ...
      control%termination = REDUCED_SIZE
...
!     Policy for the multipliers of inactive constraints
      ...
      control%inactive_y = LEAVE_AS_IS
...
!    Policy for the multipliers of inactive bounds
      ...
      control%inactive_z = LEAVE_AS_IS
...
!     Final status of the bounds on the variables
      ...
      control%final_x_bounds = TIGHTEST
...
!     Final status of the bounds on the dual variables
      ...
      control%final_z_bounds = TIGHTEST
...
!     Final status of the bounds on the constraints
      ...
      control%final_c_bounds = TIGHTEST
...
!     Final status of the bounds on the multipliers
      ...
      control%final_y_bounds = TIGHTEST
...
!     Request for dual transformations
      ...
      control%dual_transformations = .TRUE.
...
!===============================================================================
      ...
      SUBROUTINE PRESOLVE_read_specfile( device, control, inform )
