...
C Thread-safe addaptation by Nick Gould, March 2006
      ...
      RETURN
      END
      SUBROUTINE MA33AD(NN,ICN,A,LICN,LENR,LENRL,IDISP,IP,IQ,IRN,LIRN,
     ...
     +        LENRL(NN),NEXTC(NN),NEXTR(NN)
      DOUBLE PRECISION CNTL(5), RINFO(5)
  ...
  350     CONTINUE
          ...
          DO 390 I = ISTART,ILAST
            ...
            CALL MA33DD(A,ICN,IPTR(ISTART),N,IACTIV,ITOP,.TRUE.,INFO)
            IF (IACTIV-IBEG.GE.LENRL(I)) GO TO 360
  ...
  770         CONTINUE
              ...
              CALL MA33DD(A,IRN,IPC(ISTART),N,DISPC,LIRN,.FALSE.,INFO)
              IF (JROOM.LT.DISPC) GO TO 780
              JROOM = DISPC - 1
...
99992 FORMAT (' To continue set LIRN to at least ',I8)
      END
      SUBROUTINE MA33BD(N,ICN,A,LICN,LENR,LENRL,IDISP,IP,IQ,W,IW,IFLAG,
     *                  ICNTL,CNTL,RINFO)
...
99999 FORMAT (' Error return from MA33B/BD singularity detected in',
     +       ' row',I8)
      END
      SUBROUTINE MA33CD(N,ICN,A,LICN,LENR,LENRL,LENOFF,IDISP,IP,IQ,X,W,
     +                  MTYPE,RINFO)
  ...
  320 RETURN
      END
      SUBROUTINE MA33DD(A,ICN,IPTR,N,IACTIV,ITOP,REALS,INFO)
