...
!    ---------------------------------------------------------
!    | CUTEr/AMPL interface to QPA & LSQP, interior-point    |
...
!  Scalars
      ...
      INTEGER :: n, m, ir, ic, ifail, nmax, mmax, la, lh, liw, iores
!     INTEGER :: np1, npm
      ...
      INTEGER :: alloc_stat, newton, nmods, A_ne, H_ne, smt_stat
...
!  Set all default values, and override defaults if requested
      ...
      IF ( islsqp .OR. orthproj ) THEN
        CALL LSQP_initialize( data, LSQP_control )
        IF ( is_specfile )                                                     &
      ...
      ELSE
        CALL QPB_initialize( data, QPB_control )
        IF ( is_specfile )                                                     &
...
!       QPB_control%LSQP_control%print_level = 3
      ...
      IF ( printo ) WRITE( out, 2200 ) n, m, A_ne, H_ne
...
!  Deallocate arrays from the minimization
        ...
        IF ( phase1 .OR. islsqp .OR. orthproj ) THEN
          ...
          factorization_integer = LSQP_inform%factorization_integer 
          factorization_real = LSQP_inform%factorization_real
        ...
        ELSE
          ...
          factorization_integer = QPB_inform%factorization_integer 
          factorization_real = QPB_inform%factorization_real
