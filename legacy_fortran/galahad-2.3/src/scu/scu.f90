...
!----------------------------
    ...
    TYPE, PUBLIC :: SCU_data_type
      PRIVATE
      INTEGER :: m, m_max, class, jumpto, jcol, newdia, sign_determinant
      LOGICAL :: got_factors
      REAL ( KIND = wp ) :: dianew
...
!  Allocate the data arrays Q, R and W
      ...
      IF ( matrix%class <= 2 ) THEN
        IF ( ALLOCATED( data%Q ) )                                            &
          DEALLOCATE( data%Q, STAT = info%alloc_status )
        ALLOCATE( data%Q( matrix%m_max, matrix%m_max ),                        &
                  STAT = info%alloc_status )
        IF ( info%alloc_status /= 0 ) THEN ; status = - 12 ; RETURN ; END IF
      END IF

      IF ( ALLOCATED( data%R ) ) DEALLOCATE( data%R, STAT = info%alloc_status )
      ALLOCATE( data%R( matrix%m_max * ( matrix%m_max + 1 ) / 2 ),             &
                STAT = info%alloc_status )
      ...
      IF ( ALLOCATED( data%W ) ) DEALLOCATE( data%W, STAT = info%alloc_status )
      ALLOCATE( data%W( matrix%m_max ), STAT = info%alloc_status )
      IF ( info%alloc_status /= 0 ) THEN
        IF ( matrix%class <= 2 ) DEALLOCATE( data%Q, STAT = status )
