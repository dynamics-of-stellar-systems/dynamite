...
!NOT95USE GALAHAD_CPU_time
      ...
      USE GALAHAD_SMT_double, ONLY: ULS_type => SMT_type
...
!-------------------------------------------------
      ...
      TYPE, PUBLIC :: SBLS_explicit_factors_type
        ...
        TYPE ( ULS_factors ) :: B_factors
        TYPE ( SILS_control ) :: K_control
        TYPE ( ULS_control ) :: B_control
      ...
      TYPE, PUBLIC :: SBLS_implicit_factors_type
        ...
        TYPE ( ULS_type ) :: A1, B31, P22
        TYPE ( ULS_factors ) :: A1_factors, B31_factors, P22_factors
        TYPE ( ULS_control ) :: A1_control, B31_control, P22_control
      ...
      TYPE, PUBLIC :: SBLS_null_space_factors_type
        ...
        TYPE ( ULS_type ) :: A1
        TYPE ( ULS_factors ) :: A1_factors
        TYPE ( ULS_control ) :: A1_control
...
!  Initialize control parameters and arrays used within SILS and M48
      ...
      CALL ULS_INITIALIZE( data%efactors%B_factors,                            &
                           data%efactors%B_control )
      CALL ULS_INITIALIZE( data%ifactors%A1_factors,                           &
                           data%ifactors%A1_control )
      CALL ULS_INITIALIZE( data%nfactors%A1_factors,                           &
...
!  Deallocate all arrays allocated within SILS and ULS
      ...
      CALL ULS_finalize( data%efactors%B_factors,                              &
                         data%efactors%B_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

      CALL ULS_finalize( data%ifactors%A1_factors,                             &
                         data%ifactors%A1_control, inform%alloc_status )
      IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
        inform%status = GALAHAD_error_deallocate ; RETURN
      END IF

!     CALL ULS_finalize( data%ifactors%B31_factors,                            &
!                        data%ifactors%B31_control, inform%alloc_status )
!     IF ( control%deallocate_error_fatal .AND. inform%alloc_status /= 0 ) THEN
!       inform%status = GALAHAD_error_deallocate ; RETURN
!     END IF

!     CALL ULS_finalize( data%ifactors%P22_factors,                            &
...
!     END IF
      ...
      CALL ULS_finalize( data%nfactors%A1_factors,                             &
...
!  Local variables
      ...
      TYPE ( ULS_AINFO ) :: AINFO_ULS
      TYPE ( ULS_FINFO ) :: FINFO_ULS
...
!  Find sets of basic rows and columns
            ...
            CALL SBLS_find_basis( m, n, a_ne, A, efactors%B,                   &
                                  ...
                                  control, inform, AINFO_ULS, FINFO_ULS )
...
!  1c. Solve A^1^T v_3 = r_1
        ...
        trans = 1
        CALL ULS_SOLVE( ifactors%A1, ifactors%A1_factors,                      &
                        SOL( start_1 : end_1 ),                                &
                        ifactors%SOL_perm( start_3 : end_3 ),                  &
                        ifactors%A1_control, SINFO_ULS, TRANS = trans )
        inform%uls_solve_status = SINFO_ULS%flag
...
!  2. Now form A_1^-1 v -> w
          ...
          CALL ULS_SOLVE( nfactors%A1, nfactors%A1_factors,                   &
                          nfactors%V( : nfactors%rank_a ),                    &
                          nfactors%W( : nfactors%rank_a ),                    & 
                          nfactors%A1_control, SINFO_ULS )
          inform%uls_solve_status = SINFO_ULS%flag
          IF ( inform%uls_solve_status < 0 ) THEN
...
!  5. Next form w = A_1^-T v
          ...
          CALL ULS_SOLVE( nfactors%A1, nfactors%A1_factors,                    &
                          nfactors%V( : nfactors%rank_a ),                     &
                          nfactors%W( : nfactors%rank_a ),                     &
                          nfactors%A1_control, SINFO_ULS, TRANS = trans )
          inform%uls_solve_status = SINFO_ULS%flag
          IF ( inform%uls_solve_status < 0 ) THEN
...
!  Dummy arguments
      ...
      TYPE ( ULS_type ), INTENT( inout ) :: A1
      TYPE ( SMT_type ), INTENT( INOUT ) :: A2
      TYPE ( ULS_factors ), INTENT( INOUT ) :: A1_factors
      TYPE ( ULS_control ), INTENT( INOUT ) :: A1_control
      ...
      TYPE ( SBLS_control_type ), INTENT( IN ) :: control
      TYPE ( SBLS_inform_type ), INTENT( INOUT ) :: inform
      TYPE ( ULS_AINFO ), INTENT( INOUT ) :: AINFO_ULS
      TYPE ( ULS_FINFO ), INTENT( INOUT ) :: FINFO_ULS
...
!  Factorize A1
      ...
      CALL ULS_finalize( A1_factors, A1_control, info )
      IF ( control%deallocate_error_fatal .AND.                                &
           inform%status /= GALAHAD_ok ) RETURN
      CALL ULS_INITIALIZE( A1_factors, A1_control )
      IF ( control%print_level <= 0 ) THEN
        A1_control%ldiag = 0
      ...
      CALL ULS_ANALYSE( A1, A1_factors, A1_control, AINFO_ULS, FINFO_ULS )
      inform%uls_analyse_status = AINFO_ULS%flag
      IF ( printi ) WRITE( out,                                                &
         "( A, ' ULS: analysis of A1 complete: status = ', I0 )" )             &
      ...
      IF ( inform%uls_analyse_status == 4 ) THEN
        inform%rank_def = .TRUE.
        inform%rank = AINFO_ULS%rank
      END IF

      IF ( printi ) WRITE( out, "( A, ' A1 nnz(prec,factors)', 2( 1X, I0 ))")  &
        prefix, A1%ne, FINFO_ULS%size_factor
...
!  approximation to e
      ...
      IF ( control%check_basis ) THEN
        ...
        CALL ULS_SOLVE( A1, A1_factors, RHS( : rank_a ), SOL( : rank_a ),      &
                        A1_control, SINFO_ULS )
        inform%uls_solve_status = SINFO_ULS%flag
...
!       CALL CPU_TIME( t3 )
        CALL ULS_ANALYSE( A1, A1_factors,                  &
                          A1_control, AINFO_ULS, FINFO_ULS )
!       CALL CPU_TIME( t2 )

        inform%uls_analyse_status = AINFO_ULS%flag
        IF ( printi ) WRITE( out,                                              &
           "( /, A, ' ULS: analysis of A  complete: status = ', I0 )" )        &
               prefix, inform%uls_analyse_status
...
!       CALL CPU_TIME( t3 )
        CALL ULS_SPECIAL_ROWS_AND_COLS( A1_factors, rank_a, A_COLS_basic,      &
                                        A_ROWS_basic, info )
!       CALL CPU_TIME( t2 )
...
!  Determine the nonsingular submatrix of the factorization
        ...
        CALL ULS_SPECIAL_ROWS_AND_COLS( A1_factors, rank_a, A_ROWS_basic,      &
                                        A_COLS_basic, info )
      END IF    
