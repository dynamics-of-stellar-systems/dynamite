...
CS    REAL               X( N ), G( N ), H( LH )
CD    DOUBLE PRECISION   X( N ), G( N ), H( LH )
C
C  Compute the gradient and Hessian matrix of a group partially 
C  separable function. The upper triangle of the Hessian is stored 
C  in coordinate form, i.e., the entry H(i) has row index IRNH(i) 
C  and column index ICNH(i) for i = 1, ...., NNZH.
CC
