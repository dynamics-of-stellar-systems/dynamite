...
c     double precision A(MDA,*), B(*), W(*), X(*), ZZ(*)
      ...
      double precision SM, SS, T, TEMP, TWO, UNORM, UP, WMAX, SMDUM
...
C
   ...
   90       UNORM=UNORM+A(L,J)**2
      ...
      UNORM=sqrt(UNORM)
      IF (UNORM+ABS(A(NPP1,J))*FACTOR - UNORM .gt. ZERO) then
C
...
C VERBOSE PIECE ADDED BY MC, 30/NOV/2000
      SMDUM=ZERO
      DO I=NPP1,M
          SMDUM=SMDUM+B(I)**2
      END DO
C      WRITE (*,'(2I8,E15.7)') ITER,ITMAX,SQRT(SMDUM)
C      WRITE (53,'(2I8,E15.7)') ITER,ITMAX,SQRT(SMDUM)
CCCCCCCCCCCCCCCCCCCC
