...
def main(npz, n_rows=40000):
    ...
    keep = np.concatenate([[0], 1 + rng.choice(A_full.shape[0] - 1,
                                               n_rows - 1, replace=False)])
    keep.sort()
    A = np.asfortranarray(A_full[keep])
    b = np.ascontiguousarray(b_full[keep])
    del A_full, b_full
    print(f'A {A.shape} {A.dtype}, F-contiguous={A.flags.f_contiguous}',
          flush=True)
